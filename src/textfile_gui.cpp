...
const TextfileWindow::Hyperlink *TextfileWindow::GetHyperlink(Point pt) const
{
	...
	uint window_width = this->IsWrapped() ? this->GetWidget<NWidgetCore>(WID_TF_BACKGROUND)->current_x - WidgetDimensions::scaled.frametext.Horizontal() : INT_MAX;
...
/* virtual */ void TextfileWindow::DrawWidget(const Rect &r, WidgetID widget) const
{
	...
	if (!this->IsWrapped()) fr = ScrollRect(fr, *this->hscroll, 1);
