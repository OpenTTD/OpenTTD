...
void TextfileWindow::SetupScrollbars()
{
	...
	this->hscroll->SetCount(this->IsWrapped() ? 0 : CeilDiv(this->max_width, this->resize.step_width));

	this->SetWidgetDirty(WID_TF_VSCROLLBAR);
	this->SetWidgetDirty(WID_TF_HSCROLLBAR);
	this->SetWidgetDisabledState(WID_TF_HSCROLLBAR, this->IsWrapped());
}
...
/* virtual */ void TextfileWindow::DrawWidget(const Rect &r, WidgetID widget) const
{
	if (widget == WID_TF_CAPTION) {
		if (this->reflow_iter != this->reflow_end) {
	...
	if (!this->IsWrapped()) fr = ScrollRect(fr, *this->hscroll, this->resize.step_width);
...
bool TextfileWindow::IsWrapped() const
{
