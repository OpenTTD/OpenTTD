...
public:
	...
	inline bool PfDetectDestination(Node &n)
	{
		...
		return IsValidTile(GetShipDepotInWaterRegionPatch(GetShipDepot, n.key.water_region_patch));
	}
...
public:
	...
	static std::vector<HighNode> FindShipDepotRegionPath(const Ship *v, int max_penalty)
	{
		...
		Node *node = pf.GetBestNode();
		while (node != nullptr) {
			hnode_current = node->key.water_region_patch;
			hnode_cost = node->cost;
