...
protected:
	...
	static constexpr auto m_sig_look_ahead_costs = init_lookahead_costs();
		...
		if (IsTileType(tile, MP_RAILWAY)) {
			...
			if (has_signal_against && !has_signal_along && IsOnewaySignal(tile, TrackdirToTrack(trackdir))) {
				...
				if (has_signal_against && IsPbsSignal(GetSignalType(tile, TrackdirToTrack(trackdir)))) {
					cost += n.m_num_signals_passed < LOOK_AHEAD_MAX_SIGNALS ? YAPF_RAIL_PBS_SIGNAL_BACK_PENALTY : 0;
				}
			}
		}
		return cost;
