...
public:
	...
	{
		this->water_region_corridor.clear();
		for (const HighNode &path_entry : path) this->water_region_corridor.push_back(std::get<0>(path_entry));
	}
	...
	static inline FindDepotData FindNearestDepot(const Ship *v, int max_penalty)
	{
		...
		/* Set the cost of the best path and its depot. */
		depot.tile = n->segment_last_tile;
		depot.best_length = n->cost;
		...
		depot.reverse = n->segment_last_td != td;
...
public:
	...
	inline bool PfDetectDestination(Node &n)
	{
		return IsShipDepotTile(n.key.tile) && GetShipDepotPart(n.key.tile) == DEPOT_PART_NORTH && IsTileOwner(n.key.tile, Yapf().GetVehicle()->owner);
	}
...
{
	explicit CYapfShipAnyDepot(int max_nodes) { this->max_search_nodes = max_nodes; }
};
