...
public:
	...
	static Trackdir ChooseShipTrack(const Ship *v, TileIndex tile, TrackdirBits origin_dirs, bool &path_found, ShipPathCache &path_cache, Trackdir &best_origin_dir)
	{
		...
		for (int attempt = 0; attempt < 2; ++attempt) {
			...
			pf.SetOrigin(v->tile, origin_dirs);
			pf.SetDestination(v);
			...
			if (best_origin_dir == ReverseTrackdir(trackdir)) {
				path_cache.clear();
	...
	static bool CheckShipReverse(const Ship *v, Trackdir *trackdir)
	{
		...
		if (trackdir == nullptr) {
			...
			const TrackdirBits origin_dirs = TrackdirToTrackdirBits(v->GetVehicleTrackdir()) | TrackdirToTrackdirBits(reverse_dir);
			(void)ChooseShipTrack(v, v->tile, origin_dirs, path_found, dummy_cache, best_origin_dir);
			...
			const TrackdirBits origin_dirs = DiagdirReachesTrackdirs(entry) & TrackStatusToTrackdirBits(GetTileTrackStatus(v->tile, TRANSPORT_WATER, 0, entry));
			(void)ChooseShipTrack(v, v->tile, origin_dirs, path_found, dummy_cache, best_origin_dir);
			*trackdir = path_found ? best_origin_dir : GetRandomTrackdir(origin_dirs);
			return true;
		}
	}
};
...
{
	Trackdir best_origin_dir = INVALID_TRACKDIR;
	const TrackdirBits origin_dirs = TrackdirToTrackdirBits(v->GetVehicleTrackdir()) | TrackdirToTrackdirBits(ReverseTrackdir(v->GetVehicleTrackdir()));
