...
bool FioRemove(const std::string &filename)
{
	...
	if (!std::filesystem::remove(path, error_code)) {
		if (error_code) {
			Debug(misc, Severity::Fatal, "Removing {} failed: {}", filename, error_code.message());
		} else {
			Debug(misc, Severity::Fatal, "Removing {} failed: file does not exist", filename);
		}
		return false;
	}
	return true;
...
bool TarScanner::AddFile(const std::string &filename, size_t, [[maybe_unused]] const std::string &tar_filename)
{
	...
	for (;;) { // Note: feof() always returns 'false' after 'fseek()'. Cool, isn't it?
		...
		if (std::string_view{th.magic, 5} != "ustar" && std::any_of(th.magic, last_of_th, [](auto c) { return c != 0; })) {
			...
			Debug(misc, Severity::Fatal, "The file '{}' isn't a valid tar-file", filename);
			return false;
		}
		...
		if (!size.empty()) {
			...
			if (!value.has_value()) {
				Debug(misc, Severity::Fatal, "The file '{}' has an invalid size for '{}'", filename, name);
				fclose(f);
				return false;
			}
			skip = *value;
		...
		if (fseek(f, skip, SEEK_CUR) < 0) {
			Debug(misc, Severity::Fatal, "The file '{}' can't be read as a valid tar-file", filename);
			return false;
		}
		pos += skip;
	}
...
static bool ChangeWorkingDirectoryToExecutable(std::string_view exe)
{
	...
	if (chdir(path.c_str()) != 0) {
		Debug(misc, Severity::Fatal, "Directory with the binary does not exist?");
		return false;
	}
...
void DetermineBasePaths(std::string_view exe)
{
	...
	if (cwd[0] != '\0') {
		...
		if (chdir(cwd) != 0) {
			Debug(misc, Severity::Fatal, "Failed to return to working directory!");
		}
	}
