...
/* static */ ScriptDate *ScriptSubsidy::GetExpireDate(SubsidyID subsidy_id)
{
	if (!IsValidSubsidy(subsidy_id)) return new ScriptDate(ScriptDate::DATE_INVALID, ScriptDate::DT_ECONOMY);
	...
	return ScriptDate::GetDate(ymd.year.base(), ymd.month + 1, ymd.day, ScriptDate::DT_ECONOMY);
}
