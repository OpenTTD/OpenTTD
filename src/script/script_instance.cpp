...
/* static */ bool ScriptInstance::SaveObject(HSQUIRRELVM vm, SQInteger index, int max_depth, bool test)
{
	...
	switch (sq_gettype(vm, index)) {
		...
		case OT_INSTANCE:{
			...
			try {
				ScriptObject *obj;
				obj = static_cast<ScriptObject *>(Squirrel::GetRealInstance(vm, -1, "Object"));
		...
		bool operator()(const SQSaveLoadType &type)
		{
			switch (type) {
				...
				case SQSL_INSTANCE: {
					...
					sq_pushroottable(vm);
					if (SQ_FAILED(sq_call(this->vm, 1, SQTrue, SQFalse))) throw Script_FatalError(fmt::format("Failed to instanciate '{}'", class_name));
					HSQOBJECT res;
