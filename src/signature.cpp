...
static bool ValidateChecksum(const std::string &filename, const std::string &checksum)
{
	...
	if (version == "1") {
		...
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: unknown checksum version: {}", filename);
		return false;
	}

	/* Validate the checksum is the same. */
	if (calculated_hash.empty()) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: couldn't calculate checksum for: {}", filename);
		return false;
	}
	if (calculated_hash != hash) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: checksum mismatch for: {}", filename);
		return false;
	}
...
static bool ValidateSignature(const std::string &signature, const nlohmann::json &files, const std::string &filename)
{
	...
	if (version == "1") {
		...
		if (sig_value.size() != 128 || !ConvertHexToBytes(sig_value, sig)) {
			Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: invalid signature: {}", filename);
			return false;
		}
		...
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: signature validation failed: {}", filename);
		return false;
	} else {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: unknown signature version: {}", filename);
		return false;
	}
...
static bool ValidateSchema(const nlohmann::json &signatures, const std::string &filename)
{
	if (signatures["files"].is_null()) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: no files found: {}", filename);
		return false;
	}

	if (signatures["signature"].is_null()) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: no signature found: {}", filename);
		return false;
	}

	for (auto &signature : signatures["files"]) {
		if (signature["filename"].is_null() || signature["checksum"].is_null()) {
			Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: invalid entry in files: {}", filename);
			return false;
		}
		...
		if (pos == std::string::npos) {
			Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: invalid checksum format: {}", filename);
			return false;
		}
	}
	...
	auto pos = signature.find('$');
	if (pos == std::string::npos) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: invalid signature format: {}", filename);
		return false;
	}
...
static bool _ValidateSignatureFile(const std::string &filename)
{
	...
	if (!f.has_value()) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: file not found: {}", filename);
		return false;
	}
	...
	if (len != 1) {
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: failed to read file: {}", filename);
		return false;
	}
	...
	try {
		...
		Debug(Facility::Misc, Severity::Fatal, "Failed to validate signature: not a valid JSON file: {}", filename);
		return false;
	}
