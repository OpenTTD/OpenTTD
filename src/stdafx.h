...
#if (!defined(UNIX) && !defined(__CYGWIN__) && !defined(__HAIKU__) && !defined(__MORPHOS__)) || defined(__QNXNTO__)
	typedef unsigned int uint;
#endif

#if defined(TROUBLED_INTS)
	/* NDS'/Haiku's types for uint32/int32 are based on longs, which causes
