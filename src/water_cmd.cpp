	...
	switch (GetFloodingBehaviour(tile)) {
		case FloodingBehaviour::Active: {
			...
			for (Direction dir : EnumRange(Direction::End)) {
				...
				if (!_flood_from_dirs[RemoveHalftileSlope(slope_dest) & ~SLOPE_STEEP].Test(ReverseDir(dir))) continue;
		...
		case FloodingBehaviour::DryOut: {
			Slope slope_here = RemoveHalftileSlope(std::get<Slope>(GetFoundationSlope(tile))) & ~SLOPE_STEEP;
