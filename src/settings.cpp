...
static void AILoadConfig(const IniFile &ini, std::string_view grpname)
{
	...
	for (const IniItem &item : group->items) {
		...
		if (!config->HasScript()) {
			if (item.name != "none") {
				Debug(script, Severity::Fatal, "The AI by the name '{}' was no longer found, and removed from the list.", item.name);
				continue;
			}
		}
...
static void GameLoadConfig(const IniFile &ini, std::string_view grpname)
{
	...
	if (!config->HasScript()) {
		if (item.name != "none") {
			Debug(script, Severity::Fatal, "The GameScript by the name '{}' was no longer found, and removed from the list.", item.name);
			return;
		}
	}
	if (item.value.has_value()) config->StringToSettings(*item.value);
