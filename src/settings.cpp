...
static void AILoadConfig(const IniFile &ini, const char *grpname)
{
	...
	for (const IniItem &item : group->items) {
		...
		auto sc = item.name.rfind('.');
		if (sc != std::string::npos) {
			std::string ver = item.name.substr(sc + 1);
			if (!ver.empty() && std::ranges::all_of(ver, isdigit)) {
				name = item.name.substr(0, sc);
...
static void AISaveConfig(IniFile &ini, const char *grpname)
{
	...
	for (CompanyID c = CompanyID::Begin(); c < MAX_COMPANIES; ++c) {
		...
		if (config->HasScript()) {
			name = config->GetForceExactMatch() ? fmt::format("{}.{}", name, config->GetVersion()) : config->GetName();
		} else {
			name = "none";
		}
