		...
		while ((index = (CompanyID)SlIterateArray()) != (CompanyID)-1) {
				...
				if (!config->HasScript()) {
					...
					if (!config->HasScript()) {
						if (_ai_saveload_name != "%_dummy") {
							Debug(script, Severity::Fatal, "The savegame has an AI by the name '{}', version {} which is no longer available.", _ai_saveload_name, _ai_saveload_version);
							Debug(script, Severity::Fatal, "Configuration switched to Random AI.");
						}
					} else {
						Debug(script, Severity::Fatal, "The savegame has an AI by the name '{}', version {} which is no longer available.", _ai_saveload_name, _ai_saveload_version);
						Debug(script, Severity::Fatal, "The latest version of that AI has been configured instead");
					}
				}
			}
			...
			if (!config->HasScript()) {
				...
				if (!config->HasScript()) {
					if (_ai_saveload_name != "%_dummy") {
						Debug(script, Severity::Fatal, "The savegame has an AI by the name '{}', version {} which is no longer available.", _ai_saveload_name, _ai_saveload_version);
						Debug(script, Severity::Fatal, "A random other AI will be loaded in its place.");
					} else {
						Debug(script, Severity::Fatal, "The savegame had no AIs available at the time of saving.");
						Debug(script, Severity::Fatal, "A random available AI will be loaded now.");
					}
				} else {
					Debug(script, Severity::Fatal, "The savegame has an AI by the name '{}', version {} which is no longer available.", _ai_saveload_name, _ai_saveload_version);
					Debug(script, Severity::Fatal, "The latest version of that AI has been loaded instead, but it'll not get the savegame data as it's incompatible.");
				}
