			...
			for (Train *u = t; u != nullptr; u = u->Next()) {
				...
				switch (u->subtype) {
					...
					case 4: // TS_Free_Car
						...
						break;
					default:
						SlErrorCorrupt("Invalid train subtype");
				}
			}
		}
	}
...
static void CheckValidVehicles()
{
	...
	for (const Engine *e : Engine::IterateType(VEH_TRAIN)) {
		first_engine[VEH_TRAIN] = e->index;
		break;
	}
	for (const Engine *e : Engine::IterateType(VEH_ROAD)) {
		first_engine[VEH_ROAD] = e->index;
		break;
	}
	for (const Engine *e : Engine::IterateType(VEH_SHIP)) {
		first_engine[VEH_SHIP] = e->index;
		break;
	}
	for (const Engine *e : Engine::IterateType(VEH_AIRCRAFT)) {
		first_engine[VEH_AIRCRAFT] = e->index;
		break;
	}
...
public:
	...
	void Load(Vehicle *v) const override
	{
		...
		if (IsSavegameVersionBefore(SLV_PATH_CACHE_FORMAT)) {
			...
			std::transform(std::rbegin(ship_path_td), std::rend(ship_path_td), std::back_inserter(s->path), [](Trackdir trackdir) {
				return trackdir;
			});
		}
	}
...
struct VEHSChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		while ((index = SlIterateArray()) != -1) {
			...
			switch (vtype) {
				case VEH_TRAIN:
					v = new (VehicleID(index)) Train();
					break;
				case VEH_ROAD:
					v = new (VehicleID(index)) RoadVehicle();
					break;
				case VEH_SHIP:
					v = new (VehicleID(index)) Ship();
					break;
				case VEH_AIRCRAFT:
					v = new (VehicleID(index)) Aircraft();
					break;
				case VEH_EFFECT:
					v = new (VehicleID(index)) EffectVehicle();
					break;
				case VEH_DISASTER:
					v = new (VehicleID(index)) DisasterVehicle();
					break;
				case VEH_INVALID: // Savegame shouldn't contain invalid vehicles
				default:
					SlErrorCorrupt("Invalid vehicle type");
			}
