...
static uint8_t ReadByteFromFile(LoadgameState &ls)
{
	...
	if (ls.buffer_cur >= ls.buffer_count) {
		...
		if (count == 0) {
			Debug(Facility::Oldloader, Severity::Fatal, "Read past end of file, loading failed");
			throw std::exception();
		}
...
bool LoadOldSaveGame(std::string_view file)
{
	...
	if (!ls.file.has_value()) {
		Debug(Facility::Oldloader, Severity::Fatal, "Cannot open file '{}'", file);
	...
	switch (type) {
		...
		default:
			Debug(Facility::Oldloader, Severity::Fatal, "Unknown savegame type; cannot be loaded");
			break;
	}
