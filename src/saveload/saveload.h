...
struct ChunkHandler {
	...
	{
		return std::string() + static_cast<char>(this->id >> 24) + static_cast<char>(this->id >> 16) + static_cast<char>(this->id >> 8) + static_cast<char>(this->id);
	}
};
...
public:
	...
	{
		return static_cast<const TImpl *>(this)->description;
	}

	SaveLoadCompatTable GetCompatDescription() const override
	{
		return static_cast<const TImpl *>(this)->compat_description;
	}
	...
	{
		this->Save(static_cast<TObject *>(object));
	}
	...
	{
		this->Load(static_cast<TObject *>(object));
	}
	...
	{
		this->LoadCheck(static_cast<TObject *>(object));
	}
	...
	{
		this->FixPointers(static_cast<TObject *>(object));
	}
};
...
inline constexpr bool SlCheckVarSize(SaveLoadType cmd, VarType type, size_t length, size_t size)
{
	switch (cmd) {
		case SL_VAR:
			return SlVarSize(type) == size;
		case SL_REF:
			return sizeof(void *) == size;
		case SL_STDSTR:
			return SlVarSize(type) == size;
		case SL_ARR:
			return SlVarSize(type) * length <= size; // Partial load of array is permitted.
		case SL_DEQUE:
...
public:
	...
	{
		return SlGetStructListLength(MAX_LENGTH);
	}
