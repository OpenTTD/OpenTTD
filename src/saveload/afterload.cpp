...
bool AfterLoadGame()
{
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::PauseModes)) {
		...
		Debug(net, Severity::Fatal, "The loading savegame was paused due to an error state");
		Debug(net, Severity::Fatal, "  This savegame cannot be used for multiplayer");
		/* Restore the signals */
		ResetSignalHandlers();
		return false;
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::FoundTown)) {
		for (const Depot *d : Depot::Iterate()) {
			...
			if (!IsDepotTile(tile)) {
				Debug(sl, Severity::Fatal, "Removing invalid depot {} at {}, {}", d->index, TileX(d->xy), TileY(d->xy));
				delete d;
				d = nullptr;
				continue;
			}
