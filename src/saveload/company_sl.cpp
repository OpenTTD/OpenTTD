 ...
 * @return variable index if present, otherwise std::nullopt.
 */
static std::optional<uint8_t> FindFaceVar(FaceStyle style, FaceVarType type, StringID name)
{
	auto it = std::ranges::find_if(style, [type, name](const auto &s) { return s.type == type && s.name == name; });
	if (it == std::end(style)) return std::nullopt;
	return static_cast<uint8_t>(std::distance(std::begin(style), it));
}
...
CompanyManagerFace ConvertFromOldCompanyManagerFace(uint32_t face)
{
	...
	if (auto var = FindFaceVar(style, FaceVarType::Toggle, STR_FACE_GLASSES); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, GB(face, 28, 3) <= 1);
	}
	if (auto var = FindFaceVar(style, FaceVarType::Palette, STR_FACE_EYECOLOUR); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, ClampU(GB(face, 20, 3), 5, 7) - 5);
	}
	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_CHIN); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, ScaleCompanyManagerFaceValue(style, *var, GB(face, 4, 2)));
	}
	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_EYEBROWS); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, ScaleCompanyManagerFaceValue(style, *var, GB(face, 6, 4)));
	}
	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_HAIR); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, ScaleCompanyManagerFaceValue(style, *var, GB(face, 16, 4)));
	}
	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_JACKET); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, ScaleCompanyManagerFaceValue(style, *var, GB(face, 20, 2)));
	}
	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_COLLAR); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, ScaleCompanyManagerFaceValue(style, *var, GB(face, 22, 2)));
	}
	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_GLASSES); var.has_value()) {
		SetCompanyManagerFaceBits(cmf, style, *var, GB(face, 28, 1));
	}

	uint lips = GB(face, 10, 4);
	if (cmf.style != 1 && cmf.style != 3 && lips < 4) {
		if (auto var = FindFaceVar(style, FaceVarType::Toggle, STR_FACE_MOUSTACHE); var.has_value()) {
			SetCompanyManagerFaceBits(cmf, style, *var, true);
		}
		if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_MOUSTACHE); var.has_value()) {
			SetCompanyManagerFaceBits(cmf, style, *var, std::max(lips, 1U) - 1);
		}
	} else {
		if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_LIPS); var.has_value()) {
			if (cmf.style == 0 || cmf.style == 2) {
				lips = lips * 15 / 16;
				lips -= 3;
				if (cmf.style == 2 && lips > 8) lips = 0;
			} else {
				lips = ScaleCompanyManagerFaceValue(style, *var, lips);
			}
			SetCompanyManagerFaceBits(cmf, style, *var, lips);
		}

		if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_NOSE); var.has_value()) {
			uint nose = GB(face, 13, 3);
			if (cmf.style == 1) {
				nose = (nose * 3 >> 3) * 3 >> 2; // There is 'hole' in the nose sprites for women
			} else {
				nose = ScaleCompanyManagerFaceValue(style, *var, nose);
			}
			SetCompanyManagerFaceBits(cmf, style, *var, nose);
		}
	}

	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_TIE); var.has_value()) {
		uint tie = GB(face, 24, 4);
		SetCompanyManagerFaceBits(cmf, style, *var, ScaleCompanyManagerFaceValue(style, *var, tie / 2));
	}

	if (auto var = FindFaceVar(style, FaceVarType::Sprite, STR_FACE_EARRING); var.has_value()) {
		uint earring = GB(face, 24, 4);
		if (earring < 3) { // Not all women have an earring
			if (auto has_earring = FindFaceVar(style, FaceVarType::Toggle, STR_FACE_EARRING)) {
				SetCompanyManagerFaceBits(cmf, style, *has_earring, true);
				SetCompanyManagerFaceBits(cmf, style, *var, earring);
			}
		}
	}
...
static const SaveLoad _company_desc[] = {
	...
	SLE_CONDVARNAME(CompanyProperties, face.style, "face_style", SLE_UINT32, SLV_FACE_STYLES, SL_MAX_VERSION),
