...
struct CargoSpec {
	...
	struct Iterator {
		...
		CargoSpec * operator*() const { return CargoSpec::Get(static_cast<CargoType>(this->index)); }
		Iterator & operator++() { this->index++; this->ValidateIndex(); return *this; }

	private:
		size_t index;
		void ValidateIndex() { while (this->index < CargoSpec::GetArraySize() && !(CargoSpec::Get(static_cast<CargoType>(this->index))->IsValid())) this->index++; }
	};
...
private:
	static ReferenceThroughBaseContainer<std::array<CargoSpec, NUM_CARGO>> array; ///< Array holding all CargoSpecs
...
extern ReferenceThroughBaseContainer<std::array<uint8_t, NUM_CARGO>> _sorted_cargo_types;
