 ...
 * @param src The subsidy source.
 * @returns NewsReference.
 */
NewsReference GetSourceReference(const Source &src)
{
	switch (src.type) {
		case SourceType::Industry: return static_cast<IndustryID>(src.id);
		case SourceType::Town: return static_cast<TownID>(src.id);
		default: NOT_REACHED();
	}
}
 ...
 */
StringID GetSourceFormat(const Source &src)
{
	switch (src.type) {
...
void Subsidy::AwardTo(CompanyID company)
{
	...
	EncodedString headline = GetEncodedString(STR_NEWS_SERVICE_SUBSIDY_AWARDED_HALF + _settings_game.difficulty.subsidy_multiplier,
		std::move(company_name), cs->name,
		GetSourceFormat(this->src), this->src.id,
		GetSourceFormat(this->dst), this->dst.id, _settings_game.difficulty.subsidy_duration);

	/* Add a news item */
	AddNewsItem(std::move(headline), NewsType::Subsidies, NewsStyle::Normal, {},
		GetSourceReference(this->src), GetSourceReference(this->dst));
...
void CreateSubsidy(CargoType cargo_type, Source src, Source dst)
{
	...
	EncodedString headline = GetEncodedString(STR_NEWS_SERVICE_SUBSIDY_OFFERED,
		cs->name,
		GetSourceFormat(s->src), s->src.id,
		GetSourceFormat(s->dst), s->dst.id, _settings_game.difficulty.subsidy_duration);

	AddNewsItem(std::move(headline), NewsType::Subsidies, NewsStyle::Normal, {},
		GetSourceReference(s->src), GetSourceReference(s->dst));
	SetPartOfSubsidyFlag(s->src, POS_SRC);
	SetPartOfSubsidyFlag(s->dst, POS_DST);
...
static IntervalTimer<TimerGameEconomy> _economy_subsidies_monthly({TimerGameEconomy::MONTH, TimerGameEconomy::Priority::SUBSIDY}, [](auto)
{
	...
	for (Subsidy *s : Subsidy::Iterate()) {
		if (--s->remaining == 0) {
			if (!s->IsAwarded()) {
				const CargoSpec *cs = CargoSpec::Get(s->cargo_type);
				EncodedString headline = GetEncodedString(STR_NEWS_OFFER_OF_SUBSIDY_EXPIRED,
					cs->name,
					GetSourceFormat(s->src), s->src.id,
					GetSourceFormat(s->dst), s->dst.id);
				...
				if (s->awarded == _local_company) {
					const CargoSpec *cs = CargoSpec::Get(s->cargo_type);
					EncodedString headline = GetEncodedString(STR_NEWS_SUBSIDY_WITHDRAWN_SERVICE,
						cs->name,
						GetSourceFormat(s->src), s->src.id,
						GetSourceFormat(s->dst), s->dst.id);
