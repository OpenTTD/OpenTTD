		...
		switch (widget) {
			...
			case WID_AIC_INCREASE_INTERVAL: {
				...
				if (widget == WID_AIC_DECREASE_INTERVAL) {
					new_value = std::max<int>(MIN_COMPETITORS_INTERVAL, GetGameSettings().difficulty.competitors_interval - 1);
				} else {
					new_value = std::min<int>(MAX_COMPETITORS_INTERVAL, GetGameSettings().difficulty.competitors_interval + 1);
				}
