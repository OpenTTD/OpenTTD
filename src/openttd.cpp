...
bool SafeLoad(const std::string &filename, SaveLoadOperation fop, DetailedFileType dft, GameMode newgm, Subdirectory subdir, std::shared_ptr<LoadFilter> lf = nullptr)
{
	...
	if (_network_dedicated && ogm == GameMode::Menu) {
		 ...
		 */
		Debug(Facility::Net, Severity::Fatal, "Loading requested map failed; closing server.");
		_exit_game = true;
		return false;
	}
	...
	if (_network_dedicated) {
		 ...
		 */
		Debug(Facility::Net, Severity::Fatal, "Loading game failed, so a new (random) game will be started");
