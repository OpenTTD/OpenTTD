 ...
 */
static inline void DrawMatrix(const Rect &r, Colours colour, bool clicked, uint8_t num_columns, uint8_t num_rows, uint resize_x, uint resize_y)
{
	DrawFrameRect(r.left, r.top, r.right, r.bottom, colour, (clicked) ? FR_LOWERED : FR_NONE);
...
{
	this->widget_data.matrix_columns = columns;
	this->widget_data.matrix_rows = rows;
}
...
void NWidgetLeaf::Draw(const Window *w)
{
	...
	switch (this->type) {
		...
		case WWT_MATRIX:
			DrawMatrix(r, this->colour, clicked, this->widget_data.matrix_columns, this->widget_data.matrix_rows, this->resize_x, this->resize_y);
			break;
