...
struct IndustryProductionGraphWindow : BaseGraphWindow {
			...
			BaseGraphWindow(desc, STR_JUST_COMMA)
	{
		...
		for (size_t j = 0; j < i->produced.size(); j++) {
			if (!IsValidCargoID(i->GetProduced(j).cargo)) continue;
			count++;
		}
	...
	void UpdateExcludedData()
	{
		...
		for (size_t j = 0; j < i->produced.size(); j++) {
			...
			cs = CargoSpec::Get(i->GetProduced(j).cargo);
	...
	void UpdateStatistics(bool initialize) override
	{
		...
		int index = 0;
		const Industry *i = Industry::Get(this->window_number);
		for (size_t k = 0; k < i->produced.size(); k++) {
			if (!IsValidCargoID(i->GetProduced(k).cargo)) continue;

			const CargoSpec *cs = CargoSpec::Get(i->GetProduced(k).cargo);

			this->colours[index] = cs->legend_colour;
			for (uint j = 0; j < GRAPH_NUM_MONTHS; j++) {
				this->cost[index][j] = i->GetProduced(k).history[GRAPH_NUM_MONTHS - j].production;
			}
			index++;
		}
