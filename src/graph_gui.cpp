...
struct StationGraphWindow : BaseCargoGraphWindow {
	...
	static inline constexpr GraphRange RANGE_LABELS[] = {
		{STR_GRAPH_STATION_RANGE_ACCEPTED, STR_GRAPH_STATION_RANGE_ACCEPTED_TOOLTIP},
		{STR_GRAPH_STATION_RANGE_WAITING, STR_GRAPH_STATION_RANGE_WAITING_TOOLTIP},
		{STR_GRAPH_STATION_RANGE_SUPPLY, STR_GRAPH_STATION_RANGE_SUPPLY_TOOLTIP},
		{STR_GRAPH_STATION_RANGE_LOST, STR_GRAPH_STATION_RANGE_LOST_TOOLTIP},
	};
	...
	CargoTypes GetCargoTypes(WindowNumber window_number) const override
	{
		...
		for (CargoType cargo_type : _cargo_mask) {
			if (st->goods[cargo_type].HasRating() || st->goods[cargo_type].HasHistory()) cargo_types.Set(cargo_type);
		}
		return cargo_types;
	}
	...
	void UpdateStatistics(bool initialize) override
	{
		...
		/* Reserve space for the 4 series of data for each cargo type. */
		this->data.clear();
		this->data.reserve(4 * std::ranges::count_if(st->goods, &GoodsEntry::HasHistory));
		...
		for (CargoType cargo_type : this->cargo_types) {
			...
			FillFromHistory<GRAPH_NUM_MONTHS>(ge.GetHistory().history, st->valid_cargo_history, *this->scales[this->selected_scale].history_range,
				TransformFiller{{accepted}, &GoodsEntry::CargoHistory::rx_accepted, RxDecompress},
				TransformFiller{{waiting}, &GoodsEntry::CargoHistory::rx_waiting, RxDecompress},
				TransformFiller{{supply}, &GoodsEntry::CargoHistory::rx_supply, RxDecompress},
				TransformFiller{{lost}, &GoodsEntry::CargoHistory::rx_lost, RxDecompress});
		}
...
static WindowDesc _station_graph_desc(
	WindowPosition::Automatic, "graph_station", 0, 0,
	WindowClass::StationCargoGraph, WindowClass::StationView,
	{},
