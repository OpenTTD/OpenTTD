...
struct IndustryProductionGraphWindow : BaseGraphWindow {
			...
			BaseGraphWindow(desc, STR_JUST_COMMA)
	{
		...
		for (size_t j = 0; j < INDUSTRY_NUM_OUTPUTS; j++) {
			if (!IsValidCargoID(i->produced[j].cargo)) continue;
			count++;
		}
	...
	void UpdateExcludedData()
	{
		...
		for (size_t j = 0; j < INDUSTRY_NUM_OUTPUTS; j++) {
			...
			cs = CargoSpec::Get(i->produced[j].cargo);
			if (HasBit(_legend_excluded_cargo_production_history, cs->Index())) SetBit(this->excluded_data, index);
			index++;
		}
	}
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_GRAPH_MATRIX: {
				uint row = this->vscroll->GetScrolledRowFromWidget(pt.y, this, WID_GRAPH_MATRIX);
	...
	void UpdateStatistics(bool initialize) override
	{
		...
		int index = 0;
		const Industry *i = Industry::Get(this->window_number);
		for (size_t k = 0; k < lengthof(i->produced); k++) {
			if (!IsValidCargoID(i->produced[k].cargo)) continue;

			const CargoSpec *cs = CargoSpec::Get(i->produced[k].cargo);

			this->colours[index] = cs->legend_colour;
			for (uint j = 0; j < GRAPH_NUM_MONTHS; j++) {
				this->cost[index][j] = i->produced[k].history[GRAPH_NUM_MONTHS - j].production;
			}
			index++;
		}
...
static WindowDesc _industry_production_desc(__FILE__, __LINE__,
	...
	0,
	std::begin(_nested_industry_production_widgets), std::end(_nested_industry_production_widgets)
);
