		...
		switch (widget) {
			...
			case WID_GRAPH_GRAPH: {
				...
				if (this->draw_dates) {
					...
					TimerGameEconomy::Year year = this->year;
					for (int i = 0; i < this->num_on_x_axis; i++) {
						x_label_width = std::max(x_label_width, GetStringBoundingBox(
...
struct PerformanceRatingDetailWindow : Window {
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		switch (score_type) {
			...
			case SCORE_MONEY:
			case SCORE_LOAN:
				str = STR_PERFORMANCE_DETAIL_AMOUNT_CURRENCY;
				break;
			default:
				str = STR_PERFORMANCE_DETAIL_AMOUNT_INT;
				break;
		}
		DrawString(this->score_detail_left, this->score_detail_right, text_top, GetString(str, val, needed));
	}
...
struct IndustryProductionGraphWindow : BaseGraphWindow {
	...
	{
		if (widget == WID_GRAPH_CAPTION) GetString(stringid, this->window_number);
