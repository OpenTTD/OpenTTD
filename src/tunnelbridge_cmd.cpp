...
static void DrawBridgeRoadBits(TileIndex head_tile, int x, int y, int z, int offset, bool head, bool is_custom_layout)
{
	...
	if (head || !IsInvisibilitySet(TO_BRIDGES)) {
		...
		trans_back[4] = IsTransparencySet(TO_CATENARY);
		trans_front[0] = IsTransparencySet(TO_CATENARY);
		trans_front[1] = IsTransparencySet(TO_CATENARY);
...
void DrawBridgeMiddle(const TileInfo *ti, BridgePillarFlags blocked_pillars)
{
	...
	if (transport_type == TRANSPORT_ROAD) {
		...
		if (_game_mode != GameMode::Menu && _settings_client.gui.show_track_reservation && !IsInvisibilitySet(TO_BRIDGES) && HasTunnelBridgeReservation(rampnorth)) {
			if (rti->UsesOverlay()) {
