...
void ResetObjects()
{
	...
	_object_specs[OBJECT_LIGHTHOUSE].class_index = ObjectClass::Allocate(LabelToNum("LTHS"));
	_object_specs[OBJECT_TRANSMITTER].class_index = ObjectClass::Allocate(LabelToNum("TRNS"));
...
{
	ObjectClass::Get(ObjectClass::Allocate(LabelToNum("LTHS")))->name = STR_OBJECT_CLASS_LTHS;
	ObjectClass::Get(ObjectClass::Allocate(LabelToNum("TRNS")))->name = STR_OBJECT_CLASS_TRNS;
}
