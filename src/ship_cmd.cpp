...
/** Max distance in tiles (as the crow flies) to search for depots when going for automatic servicing. */
constexpr int MAX_SHIP_DEPOT_SERVICE_SEARCH_DISTANCE = 20;
...
static std::unordered_set<int> &GetReachableWaterRegionPatchHashes(const Vehicle *v, uint max_distance)
{
	...
	patches_to_search.push_back({ start_patch, 0 });
	...
	while (!patches_to_search.empty()) {
		...
		TVisitWaterRegionPatchCallBack visitFunc = [&](const WaterRegionPatchDesc &water_region_patch) {
			...
			if (!visited_patch_hashes.contains(hash)) {
				...
				patches_to_search.push_back({ water_region_patch, distance });
			}
		};
...
static void CheckIfShipNeedsService(Vehicle *v)
{
	...
	const Depot *depot = FindClosestShipDepot(v, MAX_SHIP_DEPOT_SERVICE_SEARCH_DISTANCE);
	if (depot == nullptr) {
...
static Track ChooseShipTrack(Ship *v, TileIndex tile, TrackBits tracks, bool called_recursively = false)
{
	...
	if (v->dest_tile == 0) {
		...
		if (!called_recursively && v->current_order.IsType(OT_GOTO_DEPOT) && (v->current_order.GetDepotOrderType() & ODTFB_SERVICE)) {
			const std::unordered_set<int> &visited_patch_hashes = GetReachableWaterRegionPatchHashes(v, MAX_SHIP_DEPOT_SERVICE_SEARCH_DISTANCE);
