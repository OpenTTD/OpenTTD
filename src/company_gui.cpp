...
public:
	...
	void OnTimeout() override
	{
		...
		if (colour != this->current) {
			...
			if (this->group) {
				Command<Commands::SetGroupLivery>::Post((GroupID)this->sel, primary, this->current);
			} else {
				for (LiveryScheme scheme = LS_DEFAULT; scheme < LS_END; scheme++) {
					...
					if (HasBit(this->sel, scheme) || (this->ctrl_pressed && _livery_class[scheme] == this->lc && HasBit(_loaded_newgrf_features.used_liveries, scheme))) {
						Command<Commands::SetCompanyColour>::Post(scheme, primary, this->current);
					}
				}
			}
		}
...
public:
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		auto draw_livery = [&](std::string_view str, const Livery &livery, bool is_selected, bool is_default_scheme, int indent) {
			...
			DrawSprite(SPR_SQUARE, livery.in_use.Test(Livery::Flag::Primary) ? livery.cached_pal_1cc : c->livery[LS_DEFAULT].cached_pal_1cc, pri_squ.left, y + square_offs);
			DrawString(pri.left, pri.right, y + text_offs, (is_default_scheme || livery.in_use.Test(Livery::Flag::Primary)) ? GetColourString(livery.colour1) : STR_COLOUR_DEFAULT, is_selected ? TC_WHITE : TC_GOLD);

			/* Text below the second dropdown. */
			if (sec.right > sec.left) { // Second dropdown has non-zero size.
				DrawSprite(SPR_SQUARE, livery.in_use.Test(Livery::Flag::Secondary) ? livery.cached_pal_2cr : c->livery[LS_DEFAULT].cached_pal_2cr, sec_squ.left, y + square_offs);
