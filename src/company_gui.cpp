...
static constexpr NWidgetPart _nested_company_infrastructure_widgets[] = {
	...
	EndContainer(),
	NWidget(NWID_HORIZONTAL),
		NWidget(WWT_PANEL, COLOUR_GREY, WID_CI_LIST), SetFill(1, 0), SetResize(1, 1), SetScrollbar(WID_CI_SCROLLBAR), EndContainer(),
		NWidget(NWID_VERTICAL),
...
struct CompanyInfrastructureWindow : Window
{
	...
	enum CacheEntryType : int {
		ENTRY_HEADER, ///< section headers.
		ENTRY_BLANK_LINE, ///< blank line.
		ENTRY_LABEL_ONLY, ///< label only entry, no values.
		ENTRY_LABEL_VALUES, ///< label and values (count, cost).
		ENTRY_TOTAL_COST, ///< total maintenance cost.
	};

	/* Infrastructure count and maintenance cost cache container. */
	struct InfrastructureCache {
		CacheEntryType type;
		StringID label;
		uint count;
		Money yearly_cost;

		InfrastructureCache(CacheEntryType type, StringID label, uint count = 0, Money monthly_cost = 0) : type(type), label(label), count(count), yearly_cost(monthly_cost * 12)
		{
		}
	};
	...
	uint total_y_padding; ///< Infrastructure label size.
	...
	void UpdateWidgetSize(WidgetID widget, Dimension &size, [[maybe_unused]] const Dimension &padding, [[maybe_unused]] Dimension &fill, [[maybe_unused]] Dimension &resize) override
	{
		...
		for (const auto &ie : this->infrastructurelist) {
			...
			max_cost = std::max(max_cost, ie.yearly_cost);
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		/* set boundaries for labels, infrastructure count and cost secions to handle text overflows and text direction change */
		int label_right = rr.left + this->max_label_width;
		int cost_left = label_right + WidgetDimensions::scaled.hsep_wide;
		int cost_right = cost_left + this->max_cost_width;
		int count_left = cost_right + WidgetDimensions::scaled.hsep_wide;

		auto [first, last] = this->vscroll->GetVisibleRangeIterators(this->infrastructurelist);

		for (auto it = first; it != last; ++it, y += GetCharacterHeight(FS_NORMAL)) {
			switch (it->type) {
				case ENTRY_LABEL_VALUES:

					SetDParam(0, it->count);
					DrawString(count_left, ir.right, y, STR_JUST_COMMA, TC_WHITE, SA_RIGHT);

					[[fallthrough]];

				case ENTRY_TOTAL_COST:
					if (_settings_game.economy.infrastructure_maintenance) {

						if (it->type == ENTRY_TOTAL_COST) {
							y += WidgetDimensions::scaled.vsep_normal;
							GfxFillRect(cost_left, y, cost_right, y + WidgetDimensions::scaled.bevel.top - 1, PC_WHITE);
							y += WidgetDimensions::scaled.vsep_normal;
						}

						SetDParam(0, it->yearly_cost);
					...
					if (it->type == ENTRY_TOTAL_COST) break;

					[[fallthrough]];

				case ENTRY_LABEL_ONLY:
					DrawString(ir.left, label_right, y, it->label, TC_WHITE);
					break;
				case ENTRY_HEADER:
					DrawString(rr.left, label_right, y, it->label);
					break;
				case ENTRY_BLANK_LINE:
					break;
			}
		}
	}
