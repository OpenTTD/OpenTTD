 ...
 * @param revenue The revenue generated by \a amount, used for drawing a multiplier.
 */
static void DrawPrice(Money amount, int left, int right, int top, TextColour colour, const std::optional<Money> &revenue = {})
{
	const Money amount_at_entry{amount};
	...
	if (revenue.has_value()) {
		if (*revenue < 0 && amount_at_entry > 0) {
			// Profits are positive and expenditures are negative
			const double ratio = -double(*revenue) / double(amount_at_entry);
			const auto &multiplier = GetString(STR_FINANCES_MULTIPLIER,
							   (ratio > 1.0) ? TC_LIGHT_BLUE : TC_RED,
							   10.0 * ratio, 1);
			DrawString(left, right, top, multiplier, colour, SA_RIGHT | SA_FORCE);
		} else {
			return;
		}
	} else {
...
/**
 * In this map, if key K has value V, then the finances window shows the quotient V/K
 * in the K line as a multiplier.
 */
static const std::unordered_map<ExpensesType, ExpensesType> running_costs_to_revenue{
	{ExpensesType::TrainRun, ExpensesType::TrainRevenue},
	{ExpensesType::RoadVehRun, ExpensesType::RoadVehRevenue},
	{ExpensesType::AircraftRun, ExpensesType::AircraftRevenue},
	{ExpensesType::ShipRun, ExpensesType::ShipRevenue},
};
 ...
 * @param is_multiplier True if drawing the multiplier column, false if drawing the amount column.
 * @return The income sum of the category.
 */
static Money DrawYearCategory(const Rect &r, int start_y, const ExpensesList &list, const Expenses &tbl, bool is_multiplier = false)
{
	int y = start_y;
	Money sum = 0;
	...
	for (const ExpensesType &et : list.items) {
		...
		sum += cost;
		if (is_multiplier) {
			std::optional<Money> revenue;
			if (auto it = running_costs_to_revenue.find(et); it != running_costs_to_revenue.end())
				revenue = tbl[it->second];
			if (revenue)
				DrawPrice(cost, r.left, r.right, y, TC_BLACK, revenue);
		} else {
 ...
 * @note The environment must provide padding at the left and right of \a r.
 */
static void DrawYearColumn(const Rect &r, TimerGameEconomy::Year year, const Expenses &tbl, bool is_multiplier = false)
{
	int y = r.top;
	Money sum;

	/* Year header */
	DrawString(r.left, r.right, y, is_multiplier ? GetString(STR_EMPTY) : GetString(STR_FINANCES_YEAR, year), TC_FROMSTRING, SA_RIGHT | SA_FORCE, true);
	y += GetCharacterHeight(FontSize::Normal) + WidgetDimensions::scaled.vsep_wide;

	/* Categories */
	for (const ExpensesList &list : _expenses_list_types) {
		y += GetCharacterHeight(FontSize::Normal);
		sum += DrawYearCategory(r, y, list, tbl, is_multiplier);
...
static constexpr std::initializer_list<NWidgetPart> _nested_company_finances_widgets = {
	...
	NWidget(NWID_SELECTION, Colours::Invalid, WID_CF_SEL_PANEL),
		NWidget(WWT_PANEL, Colours::Grey),
			NWidget(NWID_HORIZONTAL), SetPadding(WidgetDimensions::unscaled.framerect), SetPIP(0, WidgetDimensions::unscaled.hsep_wide, 0),
				...
				NWidget(WWT_EMPTY, Colours::Invalid, WID_CF_EXPS_MULTIPLIER1), SetMinimalSize(0, 0), SetFill(0, 0),
				NWidget(WWT_EMPTY, Colours::Invalid, WID_CF_EXPS_PRICE2), SetMinimalSize(86, 0), SetFill(0, 0),
				NWidget(WWT_EMPTY, Colours::Invalid, WID_CF_EXPS_MULTIPLIER2), SetMinimalSize(0, 0), SetFill(0, 0),
				NWidget(WWT_EMPTY, Colours::Invalid, WID_CF_EXPS_PRICE3), SetMinimalSize(86, 0), SetFill(0, 0),
				NWidget(WWT_EMPTY, Colours::Invalid, WID_CF_EXPS_MULTIPLIER3), SetMinimalSize(0, 0), SetFill(0, 0),
			EndContainer(),
		EndContainer(),
	EndContainer(),
	NWidget(WWT_PANEL, Colours::Grey),
	...
	NWidget(NWID_SELECTION, Colours::Invalid, WID_CF_SEL_BUTTONS),
		NWidget(NWID_HORIZONTAL, NWidContainerFlag::EqualSize),
			NWidget(WWT_PUSHTXTBTN, Colours::Grey, WID_CF_INCREASE_LOAN), SetFill(1, 0), SetToolTip(STR_FINANCES_BORROW_TOOLTIP),
			NWidget(WWT_PUSHTXTBTN, Colours::Grey, WID_CF_REPAY_LOAN), SetFill(1, 0), SetToolTip(STR_FINANCES_REPAY_TOOLTIP),
...
struct CompanyFinancesWindow : Window {
	...
	void UpdateWidgetSize(WidgetID widget, Dimension &size, [[maybe_unused]] const Dimension &padding, [[maybe_unused]] Dimension &fill, [[maybe_unused]] Dimension &resize) override
	{
		switch (widget) {
			...
			case WID_CF_EXPS_MULTIPLIER1:
			case WID_CF_EXPS_MULTIPLIER2:
			case WID_CF_EXPS_MULTIPLIER3:
				size.height = GetTotalCategoriesHeight();
				// Depending on play style and effective NewGRFs, the widest multipliers could be as narrow as
				// "x5", or as wide as "x700" or more. Hence, compute the required width dynamically.
				{
					int period = widget - WID_CF_EXPS_MULTIPLIER1;
					// No need to check first_visible, since we check the denominator for being non-zero.
					const Company *c = Company::Get((CompanyID)this->window_number);
					const Expenses &tbl = c->yearly_expenses[NUM_PERIODS - period - 1];
					size.width = 0;
					for (const auto &[expenditures, revenues] : running_costs_to_revenue) {
						if (tbl[expenditures]) {
							double ratio = -double(tbl[revenues]) / double(tbl[expenditures]);
							const auto &multiplier = GetString(STR_FINANCES_MULTIPLIER,
										  TextColour(3), // arbitrary valid value
										  10.0 * ratio, 1);
							size.width = std::max(size.width, GetStringBoundingBox(multiplier).width + padding.width);
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		switch (widget) {
			...
			case WID_CF_EXPS_PRICE2:
			case WID_CF_EXPS_PRICE3: {
				int period = widget - (is_multiplier ? WID_CF_EXPS_MULTIPLIER1 : WID_CF_EXPS_PRICE1);
				if (period < this->first_visible) break;

				const Company *c = Company::Get(this->window_number);
				const auto &expenses = c->yearly_expenses[NUM_PERIODS - period - 1];
				DrawYearColumn(r, TimerGameEconomy::year - (NUM_PERIODS - period - 1), expenses, is_multiplier);
				break;
			}
...
static constexpr std::initializer_list<NWidgetPart> _nested_company_infrastructure_widgets = {
	...
	EndContainer(),
	NWidget(NWID_HORIZONTAL),
		NWidget(WWT_PANEL, Colours::Grey, WID_CI_LIST), SetFill(1, 1), SetResize(0, 1),
...
struct CompanyInfrastructureWindow : Window
{
	...
	void FindWindowPlacementAndResize(int def_width, int def_height, bool allow_resize) override
	{
		if (def_height == 0) {
			...
			int rows = (this->GetWidget<NWidgetBase>(WID_CI_LIST)->current_y - WidgetDimensions::scaled.framerect.Vertical()) / GetCharacterHeight(FontSize::Normal);
			int delta = std::min(20, static_cast<int>(std::size(this->list))) - rows;
			def_height = this->height + delta * GetCharacterHeight(FontSize::Normal);
		}
