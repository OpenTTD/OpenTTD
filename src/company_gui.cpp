...
void DrawCompanyManagerFace(const CompanyManagerFace &cmf, Colours colour, const Rect &r)
{
	...
	auto style = GetCompanyManagerFaceStyle(cmf.style);

	/* First determine which parts are enabled. */
	uint64_t active_vars = GetActiveFaceVars(cmf, style);

	std::unordered_map<uint8_t, PaletteID> palettes;

	/* Second, get palettes. */
	for (auto var : SetBitIterator(active_vars)) {
		if (style[var].type != FaceVarType::Palette) continue;

		PaletteID pal = PAL_NONE;
		switch (GetCompanyManagerFaceBits(cmf, style, var)) {
			default: NOT_REACHED();
			case 0: pal = PALETTE_TO_BROWN; break;
			case 1: pal = PALETTE_TO_BLUE;  break;
			case 2: pal = PALETTE_TO_GREEN; break;
		}
		for (uint8_t affected_var : SetBitIterator(std::get<uint64_t>(style[var].data))) {
	...
	for (auto var : SetBitIterator(active_vars)) {
		if (style[var].type != FaceVarType::Sprite) continue;

		auto it = palettes.find(var);
		PaletteID pal = (it == std::end(palettes)) ? PAL_NONE : it->second;
		DrawSprite(GetCompanyManagerFaceSprite(cmf, style, var), pal, x, y);
	}
}
...
class SelectCompanyManagerFaceWindow : public Window
{
	...
	uint selected_var = UINT_MAX;
	uint click_state = 0;
	int line_height = 0;

	void UpdateData()
	{
		auto style = GetCompanyManagerFaceStyle(this->face.style);
		ScaleAllCompanyManagerFaceBits(this->face, style);
		this->active_parts = GetActiveFaceVars(this->face, style);

		/* Exclude active parts which have no string. */
		for (auto var : SetBitIterator(this->active_parts)) {
			if (style[var].name == STR_NULL) ClrBit(this->active_parts, var);
		}

		this->GetScrollbar(WID_SCMF_PARTS_SCROLLBAR)->SetCount(CountBits(this->active_parts));
	}
...
public:
	...
	static uint GetMaximumFacePartsWidth()
	{
		...
		for (uint style_index = 0; style_index != GetNumCompanyManagerFaceStyles(); ++style_index) {
			auto style = GetCompanyManagerFaceStyle(style_index);
	...
	void UpdateWidgetSize(WidgetID widget, Dimension &size, [[maybe_unused]] const Dimension &padding, [[maybe_unused]] Dimension &fill, [[maybe_unused]] Dimension &resize) override
	{
		switch (widget) {
			...
			case WID_SCMF_PARTS:
				...
				size.height = std::max(size.height, resize.height * 5);
				break;
		}
	}
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		switch (widget) {
			...
			case WID_SCMF_PARTS: {
				...
				auto sbi = SetBitIterator(this->active_parts);
				auto [first, last] = this->GetScrollbar(WID_SCMF_PARTS_SCROLLBAR)->GetVisibleRangeIterators(sbi);
				for (auto it = first; it != last; ++it) {
					const auto &s = style[*it];

					Rect br = ir.CentreTo(ir.Width(), SETTING_BUTTON_HEIGHT).WithWidth(SETTING_BUTTON_WIDTH, rtl);
					Rect tr = ir.Shrink(RectPadding::zero, WidgetDimensions::scaled.matrix).CentreTo(ir.Width(), GetCharacterHeight(FS_NORMAL)).Indent(SETTING_BUTTON_WIDTH + WidgetDimensions::scaled.hsep_wide, rtl);

					uint val = GetCompanyManagerFaceBits(this->face, style, *it);
					if (s.type == FaceVarType::Toggle) {
						DrawBoolButton(br.left, br.top, COLOUR_YELLOW, COLOUR_GREY, val == 1, true);
						DrawString(tr, GetString(STR_FACE_SETTING_TOGGLE, s.name, val == 1 ? STR_FACE_YES : STR_FACE_NO), TC_WHITE);
					} else {
						DrawArrowButtons(br.left, br.top, COLOUR_YELLOW, this->selected_var == *it ? this->click_state : 0, true, true);
						DrawString(tr, GetString(STR_FACE_SETTING_NUMERIC, s.name, val + 1, s.valid_values), TC_WHITE);
					}
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			/* Load button */
			case WID_SCMF_LOAD: {
				auto cmf = ParseCompanyManagerFaceString(_company_manager_face);
			...
			case WID_SCMF_FACECODE:
				ShowQueryString(FormatCompanyManagerFaceString(this->face), STR_FACE_FACECODE_CAPTION, 128, this, CS_NUMERAL, {});
				break;

			/* Save button */
			case WID_SCMF_SAVE:
				_company_manager_face = FormatCompanyManagerFaceString(this->face);
				ShowErrorMessage(GetEncodedString(STR_FACE_SAVE_DONE), {}, WL_INFO);
				break;

			/* Randomize face button */
			case WID_SCMF_RANDOM_NEW_FACE:
				this->face.style = _interactive_random.Next(GetNumCompanyManagerFaceStyles());
				RandomCompanyManagerFaceBits(this->face, GetCompanyManagerFaceStyle(this->face.style), _interactive_random);
				this->UpdateData();
				this->SetDirty();
				break;
			...
			case WID_SCMF_PARTS: {
				...
				auto sbi = SetBitIterator(this->active_parts);
				auto it = this->GetScrollbar(WID_SCMF_PARTS_SCROLLBAR)->GetScrolledItemFromWidget(sbi, pt.y, this, widget, 0, this->line_height);
				if (it != std::end(sbi)) this->selected_var = *it;

				if (this->selected_var == UINT_MAX) break;

				auto style = GetCompanyManagerFaceStyle(this->face.style);
				const auto &s = style[this->selected_var];
				if (s.type == FaceVarType::Toggle) {
					if (!IsInsideBS(pt.x, br.left, SETTING_BUTTON_WIDTH)) break;
					IncreaseCompanyManagerFaceBits(this->face, style, this->selected_var, 1);
				} else {
					if (IsInsideBS(pt.x, br.left, SETTING_BUTTON_WIDTH / 2)) {
						IncreaseCompanyManagerFaceBits(this->face, style, this->selected_var, -1);
						this->click_state = 1;
	...
	void OnQueryTextFinished(std::optional<std::string> str) override
	{
		...
		auto cmf = ParseCompanyManagerFaceString(*str);
		if (cmf.has_value()) {
			this->face = *cmf;
