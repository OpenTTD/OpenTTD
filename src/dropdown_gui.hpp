...
concept DropdownWindowType = std::derived_from<TDropdownWindow, DropdownWindow> && requires (int window_id, Window *parent, DropDownList &&list, int selected, WidgetID button, const Rect wi_rect, bool instant_close, Colours wi_colour, bool persist) {
	{ TDropdownWindow(window_id, parent, std::move(list), selected, button, wi_rect, instant_close, wi_colour, persist) };
};
...
void ShowCustomDropdownList(Window *w, DropDownList &&list, int selected, WidgetID button, uint width = 0, bool instant_close = false, bool persist = false)
{
	...
	new TDropdownWindow(0, w, std::move(list), selected, button, wi_rect, instant_close, wi_colour, persist);
}
