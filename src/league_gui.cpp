...
public:
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		if (!lt->header.empty()) {
			ir.top = DrawStringMultiLine(ir.left, ir.right, ir.top, UINT16_MAX, GetString(STR_JUST_RAW_STRING, lt->header), TC_BLACK) + WidgetDimensions::scaled.vsep_wide;
		}
		...
		if (!lt->footer.empty()) {
			...
			ir.top = DrawStringMultiLine(ir.left, ir.right, ir.top, UINT16_MAX, GetString(STR_JUST_RAW_STRING, lt->footer), TC_BLACK);
		}
	}
