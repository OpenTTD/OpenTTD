...
static void DrawTrackDetails(const TileInfo *ti, const RailTypeInfo *rti, PaletteID pal)
{
	...
	uint num_sprites = 0;
	PalSpriteID psid{
		.sprite = GetCustomRailSprite(rti, ti->index, RTSG_FENCES, IsHalftileSlope(ti->tileh) ? TCX_UPPER_HALFTILE : TCX_NORMAL, &num_sprites),
		.pal = pal,
	};
	if (psid.sprite == 0) {
...
static void GetTileDesc_Rail(TileIndex, Tile tile, TileDesc &td)
{
