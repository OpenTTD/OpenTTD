...
std::vector<TownID> GetRandomisedTownList()
{
	...
	for (const Town *t : Town::Iterate()) towns.push_back(t->index);
...
{
	for (TownID town : GetRandomisedTownList()) {
...
void GenerateObjects()
{
	...
	for (const auto &spec : ObjectSpec::Specs()) {
		...
		if (!spec.flags.Any({ObjectFlag::PlaceNearTowns, ObjectFlag::PlaceNearCoast})) {
