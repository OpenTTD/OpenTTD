...
static CommandCost BuildReplacementVehicle(Vehicle *old_veh, Vehicle **new_vehicle, bool part_of_chain, DoCommandFlags flags)
{
	...
	*new_vehicle = new_veh;
	...
	if (refit_cargo != CARGO_NO_REFIT) {
		...
		cost.AddCost(std::get<CommandCost>(Command<Commands::RefitVehicle>::Do(DoCommandFlag::Execute, new_veh->index, refit_cargo, subtype, false, false, 0)));
...
static CommandCost CopyHeadSpecificThings(Vehicle *old_head, Vehicle *new_head, DoCommandFlags flags)
{
	...
	if (cost.Succeeded() && old_head != new_head) cost.AddCost(std::get<CommandCost>(Command<Commands::AddVehicleToGroup>::Do(DoCommandFlag::Execute, old_head->group_id, new_head->index, false, VehicleListIdentifier{})));
