...
struct EnginePreviewWindow : Window {
	...
	void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize) override
	{
		...
		size->height = GetStringHeight(STR_ENGINE_PREVIEW_DESCRIPTION, size->width) + WidgetDimensions::scaled.vsep_wide + FONT_HEIGHT_NORMAL + this->vehicle_space;
		SetDParam(0, engine);
	...
	void DrawWidget(const Rect &r, int widget) const override
	{
		...
		EngineID engine = this->window_number;
		SetDParam(0, GetEngineCategoryName(engine));
		int y = DrawStringMultiLine(r, STR_ENGINE_PREVIEW_DESCRIPTION, TC_FROMSTRING, SA_HOR_CENTER | SA_TOP) + WidgetDimensions::scaled.vsep_wide;
