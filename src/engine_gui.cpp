...
static constexpr NWidgetPart _nested_engine_preview_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(NWID_VERTICAL),
			NWidget(NWID_SELECTION, INVALID_COLOUR, WID_EP_LIST_TOGGLE_CONTAINER),
				NWidget(WWT_PUSHARROWBTN, COLOUR_LIGHT_BLUE, WID_EP_TOGGLE_LIST), SetMinimalSize(9, 12), SetResize(0, 1), SetFill(0, 1), SetArrowWidgetTypeTip(AWV_INCREASE),
				NWidget(WWT_PUSHARROWBTN, COLOUR_LIGHT_BLUE, WID_EP_TOGGLE_LIST), SetMinimalSize(9, 12), SetResize(0, 1), SetFill(0, 1), SetArrowWidgetTypeTip(AWV_DECREASE),
			EndContainer(),
		EndContainer(),
...
struct EnginePreviewWindow : BuildVehicleWindow {
	...
	/**
	 * Updates list containers to match is_list_visible.
	 */
	void UpdateListContainers()
	{
		this->GetWidget<NWidgetStacked>(WID_EP_LIST_CONTAINER)->SetDisplayedPlane(this->is_list_visible ? 0 : SZSP_VERTICAL);
		this->GetWidget<NWidgetStacked>(WID_EP_LIST_TOGGLE_CONTAINER)->SetDisplayedPlane(this->is_list_visible);
	}
	...
	/** Updates capacity of engine list scrollbar. */
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_EP_ACCEPT_ALL:
				for (uint8_t length = this->filtered_eng_list.size(); length != 0; --length) {
					this->sel_engine = this->filtered_eng_list[0].engine_id;
