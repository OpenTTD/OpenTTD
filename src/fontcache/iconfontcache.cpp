...
static void DrawIconGlyph(const IconGlyph ig, const Rect &r)
{
	struct visitor {
		...
		{
			SpriteID spriteid = CargoSpec::Get(cargotype)->GetCargoIcon();
			DrawSprite(spriteid, PAL_NONE, CentreBounds(r.left, r.right, GetSpriteSize(spriteid).width), r.top);
		}
		...
		{
			// AutoRestoreBackup backup{_gui_zoom, ZOOM_LVL_GUI + 1};
			DrawVehicleEngine(r.left, r.right, (r.left + r.right + 1) / 2, (r.top + r.bottom + 1) / 2, engine, PAL_NONE, EIT_PREVIEW);
		}
	};
...
static IconFontCacheFactory s_sprite_fontcache_factory;
