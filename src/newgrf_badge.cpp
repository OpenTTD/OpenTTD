...
static PalSpriteID GetBadgeSprite(const Badge &badge, GrfSpecFeature feature, std::optional<TimerGameCalendar::Date> introduction_date, PaletteID remap)
{
	...
	PaletteID pal = HasFlag(badge.flags, BadgeFlags::UseCompanyColour) ? remap : PAL_NONE;
...
/**
 * Test if one of the lists badges matches the filtered badge list.
 * @param badges
...
int DrawBadgeNameList(Rect r, std::span<const BadgeID> badges, GrfSpecFeature)
{
	...
	for (const BadgeClassID &class_index : classes) {
		...
		for (const BadgeID &index : badges) {
			...
			if (!s.empty()) {
				if (HasFlag(badge->flags, BadgeFlags::NameListFirstOnly)) continue;
				s += ", ";
			}
			AppendStringInPlace(s, badge->name);
			if (HasFlag(badge->flags, BadgeFlags::NameListStop)) break;
		}
