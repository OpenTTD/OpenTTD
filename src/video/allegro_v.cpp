...
static bool CreateMainSurface(uint w, uint h)
{
	...
	if (set_gfx_mode(_fullscreen ? GFX_AUTODETECT_FULLSCREEN : GFX_AUTODETECT_WINDOWED, w, h, 0, 0) != 0) {
		Debug(Facility::Driver, Severity::Fatal, "Allegro: Couldn't allocate a window to draw on '{}'", allegro_error);
		return false;
	}
...
#if 0
	Debug(Facility::Driver, Severity::Fatal, "Scancode character pressed {}", scancode);
	Debug(Facility::Driver, Severity::Fatal, "Unicode character pressed {}", unicode);
#endif
