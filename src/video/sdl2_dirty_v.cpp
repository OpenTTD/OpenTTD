...
/** @file sdl2_dirty_v.cpp Implementation of the SDL2 video driver, patched to visualise dirty screen areas. */
...
static FVideoDriver_SDL_Dirty iFVideoDriver_SDL_Dirty;
...
void VideoDriver_SDL_Dirty::MakeDirty(int left, int top, int width, int height)
{
...
#	include "../table/string_colours.h"
	...
	int x1 = left, y1 = top, x2 = left + width - 1, y2 = top + height - 1;
	blitter->DrawLine(_screen.dst_ptr, x1, y1, x2, y1, _screen.width, _screen.height, colour, 1, 2);
	blitter->DrawLine(_screen.dst_ptr, x1, y2, x2, y2, _screen.width, _screen.height, colour, 1, 2);
	blitter->DrawLine(_screen.dst_ptr, x1, y1, x1, y2, _screen.width, _screen.height, colour, 1, 2);
	blitter->DrawLine(_screen.dst_ptr, x2, y1, x2, y2, _screen.width, _screen.height, colour, 1, 2);
	blitter->DrawLine(_screen.dst_ptr, x1, y1, x2, y2, _screen.width, _screen.height, colour, 1, 2);
	blitter->DrawLine(_screen.dst_ptr, x2, y1, x1, y2, _screen.width, _screen.height, colour, 1, 2);
...
bool VideoDriver_SDL_Dirty::CreateMainSurface(uint w, uint h, bool resize)
{
...
bool VideoDriver_SDL_Dirty::ClaimMousePointer()
{
	SDL_ShowCursor(0);
	return true;
}
 ...
 */
void VideoDriver_SDL_Dirty::EditBoxGainedFocus()
{
 ...
 */
void VideoDriver_SDL_Dirty::EditBoxLostFocus()
{
	if (this->edit_box_focused) {
		SDL_StopTextInput();
...
int VideoDriver_SDL_Dirty::PollEvent()
{
	SDL_Event ev;
...
const char *VideoDriver_SDL_Dirty::Start(const StringList &parm)
{
...
void VideoDriver_SDL_Dirty::Stop()
{
...
void VideoDriver_SDL_Dirty::MainLoop()
{
...
bool VideoDriver_SDL_Dirty::ChangeResolution(int w, int h)
{
...
bool VideoDriver_SDL_Dirty::ToggleFullscreen(bool fullscreen)
{
	std::unique_lock<std::recursive_mutex> lock;
	if (_draw_mutex != nullptr) lock = std::unique_lock<std::recursive_mutex>(*_draw_mutex);
...
bool VideoDriver_SDL_Dirty::AfterBlitterChange()
{
	int w, h;
...
void VideoDriver_SDL_Dirty::AcquireBlitterLock()
{
	if (_draw_mutex != nullptr) _draw_mutex->lock();
}

void VideoDriver_SDL_Dirty::ReleaseBlitterLock()
{
