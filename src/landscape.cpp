...
static uint8_t _cur_tile_loop_pattern[TILE_LOOP_PATTERN_SIZE]; // Tile loop pattern, changes every 256 ticks
...
bool WasTileIteratedThisCycle(TileIndex tile)
{
	...
	return (_cur_tile_loop_pattern[pattern_index] <= TimerGameTick::counter % 256);
}
...
void GenerateTileLoopPattern() {
	...
	static_assert(TILE_LOOP_PATTERN_SIZE == 4096);  // different size requires different feedback
	...
	for (uint i = 1; i < TILE_LOOP_PATTERN_SIZE; i++) {
		...
		_cur_tile_loop_pattern[lfsr_current] = i / (TILE_LOOP_PATTERN_SIZE / 256);
	}
}
...
void RunTileLoop()
{
	...
	/* Every 256 ticks generate a new tile pattern just to make it a bit more random. */
