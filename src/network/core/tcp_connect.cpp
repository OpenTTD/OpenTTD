...
void TCPConnecter::Connect(addrinfo *address)
{
	...
	if (sock == INVALID_SOCKET) {
		Debug(net, Severity::Fatal, "Could not create {} {} socket: {}", NetworkAddress::SocketTypeAsString(address->ai_socktype), NetworkAddress::AddressFamilyAsString(address->ai_family), NetworkError::GetLast().AsString());
		return;
	}

	if (!SetReusePort(sock)) {
		Debug(net, Severity::Fatal, "Setting reuse-port mode failed: {}", NetworkError::GetLast().AsString());
	}
	...
	if (!SetNonBlocking(sock)) {
		Debug(net, Severity::Fatal, "Setting non-blocking mode failed: {}", NetworkError::GetLast().AsString());
	}
...
void TCPConnecter::Resolve()
{
	...
	if (!getaddrinfo_timeout_error_shown && duration >= std::chrono::seconds(5)) {
		Debug(net, Severity::Fatal, "getaddrinfo() for address \"{}\" took {} seconds", this->connection_string, duration.count());
		Debug(net, Severity::Fatal, "  This is likely an issue in the DNS name resolver's configuration causing it to time out");
		getaddrinfo_timeout_error_shown = true;
	}

	if (error != 0) {
		Debug(net, Severity::Fatal, "Failed to resolve DNS for {}", this->connection_string);
...
bool TCPConnecter::CheckActivity()
{
	...
	if (n == 0) {
		...
		Debug(net, Severity::Fatal, "Timeout while connecting to {}", this->connection_string);
