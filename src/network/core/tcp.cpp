	...
	while (!this->packet_queue.empty()) {
		...
		if (res == -1) {
			...
			if (!err.WouldBlock()) {
				...
				if (!closing_down) {
					Debug(Facility::Net, Severity::Fatal, "Send failed: {}", err.AsString());
...
std::unique_ptr<Packet> NetworkTCPSocketHandler::ReceivePacket()
{
		...
		while (p.RemainingBytesToTransfer() != 0) {
			...
			if (res == -1) {
				...
				if (!err.WouldBlock()) {
					...
					if (!err.IsConnectionReset()) Debug(Facility::Net, Severity::Fatal, "Recv failed: {}", err.AsString());
					this->CloseConnection();
					return nullptr;
				}
	...
	if (!p.PrepareToRead()) {
		Debug(Facility::Net, Severity::Fatal, "Invalid packet received (too small / decryption error)");
		this->CloseConnection();
		return nullptr;
	}
