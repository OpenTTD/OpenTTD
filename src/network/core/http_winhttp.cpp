...
void NetworkHTTPRequest::WinHttpCallback(DWORD code, void *info, DWORD length)
{
	...
	switch (code) {
		...
		case WINHTTP_CALLBACK_STATUS_REDIRECT:
			...
			if (this->depth++ > 5) {
				Debug(net, Severity::Fatal, "HTTP request failed: too many redirects");
		...
		case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE:
		{
			...
			if (status_code >= 400) {
				...
				Debug(net, status_code == HTTP_429_TOO_MANY_REQUESTS ? Severity::Error : Severity::Fatal, "HTTP request failed: status-code {}", status_code);
				this->callback.OnFailure();
				this->finished = true;
				return;
			}
		...
		case WINHTTP_CALLBACK_STATUS_REQUEST_ERROR:
			Debug(net, Severity::Fatal, "HTTP request failed: {}", GetLastErrorAsString());
			this->callback.OnFailure();
			this->finished = true;
			break;

		default:
			Debug(net, Severity::Fatal, "HTTP request failed: unexpected callback code 0x{:x}", code);
			this->callback.OnFailure();
			this->finished = true;
			return;
	}
