...
void NetworkUDPSocketHandler::HandleUDPPacket(Packet &p, NetworkAddress &client_addr)
{
	...
	switch (type) {
		...
		default:
			Debug(net, Severity::Fatal, "[udp] Received invalid packet type {} from {}", type, client_addr.GetAddressAsString());
			break;
	}
}
...
{
	Debug(net, Severity::Fatal, "[udp] Received packet type {} on wrong port from {}", type, client_addr.GetAddressAsString());
}
