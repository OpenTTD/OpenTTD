...
void ClientNetworkContentSocketHandler::RequestQueuedContentInfo()
{
	...
	/* Wait til we've briefly stopped receiving data (which will contain more content) before making the request. */
	if (std::chrono::steady_clock::now() <= this->last_activity + CONTENT_QUEUE_TIMEOUT) {
		_request_queue_timeout.Reset();
		return;
	}

	/* We're doing it. Move the queue locally so more ids can be queued for later. */
	ContentIDList queue = std::move(this->queued);
