...
void NetworkGameLoop()
{
	...
	if (_network_server) {
		...
		if (!f.has_value() && next_date == 0) {
			Debug(desync, Severity::Fatal, "Cannot open commands.log");
		...
		while (f.has_value() && !feof(*f)) {
			if (TimerGameEconomy::date == next_date && TimerGameEconomy::date_fract == next_date_fract) {
				if (cp != nullptr) {
					...
					Debug(desync, Severity::Fatal, "Injecting: {:08x}; {:02x}; {:02x}; {:08x}; {} ({})", TimerGameEconomy::date, TimerGameEconomy::date_fract, _current_company.base(), cp->cmd, FormatArrayAsHex(cp->data), GetCommandName(cp->cmd));
					delete cp;
					cp = nullptr;
				}
				if (check_sync_state) {
					if (sync_state[0] == _random.state[0] && sync_state[1] == _random.state[1]) {
						Debug(desync, Severity::Fatal, "Sync check: {:08x}; {:02x}; match", TimerGameEconomy::date, TimerGameEconomy::date_fract);
					} else {
						Debug(desync, Severity::Fatal, "Sync check: {:08x}; {:02x}; mismatch expected {{{:08x}, {:08x}}}, got {{{:08x}, {:08x}}}",
			...
			if (TimerGameEconomy::date > next_date || (TimerGameEconomy::date == next_date && TimerGameEconomy::date_fract > next_date_fract)) {
				Debug(desync, Severity::Fatal, "Skipping to next command at {:08x}:{:02x}", next_date, next_date_fract);
				if (cp != nullptr) {
					delete cp;
					cp = nullptr;
				}
			...
			if (consumer.ReadIf("cmd: ")
				...
				next_date_fract = consumer.ReadIntegerBase<uint32_t>(16);
				assert(valid);
				Debug(desync, Severity::Fatal, "Injecting pause for join at {:08x}:{:02x}; please join when paused", next_date, next_date_fract);
				cp = new CommandPacket();
				...
				Debug(desync, Severity::Fatal, "Skipping replay of failed command: {}", consumer.Read(StringConsumer::npos));
#endif
			} else {
				/* Can't parse a line; what's wrong here? */
				Debug(desync, Severity::Fatal, "Trying to parse: {}", consumer.Read(StringConsumer::npos));
				NOT_REACHED();
			}
		}
		if (f.has_value() && feof(*f)) {
			Debug(desync, Severity::Fatal, "End of commands.log");
			f.reset();
		}
#endif /* DEBUG_DUMP_COMMANDS */
