...
struct VehicleScopeResolver : public ScopeResolver {
	 ...
	 */
	VehicleScopeResolver(ResolverObject &ro, EngineID engine_type, const Vehicle *v, bool rotor_in_gui) : ScopeResolver(ro), v(v), self_type(engine_type), rotor_in_gui(rotor_in_gui) {}
...
struct VehicleResolverObject : public SpecializedResolverObject<VehicleRandomTriggers> {
	...
	VehicleResolverObject(EngineID engine_type, const Vehicle *v, WagonOverride wagon_override, bool rotor_in_gui = false, CallbackID callback = CBID_NO_CALLBACK, uint32_t callback_param1 = 0,
