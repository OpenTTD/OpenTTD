...
static void FormatString(StringBuilder &builder, std::string_view str_arg, StringParameters &args, uint orig_case_index, bool game_script, bool dry_run)
{
	...
	str_stack.emplace(&*str_arg.begin(), &*str_arg.end(), orig_first_param_offset, orig_case_index);
		...
		try {
			...
			switch (b) {
				...
				case SCC_NEWGRF_STRINL: {
					...
					str_stack.emplace(&*ptr.begin(), &*ptr.end(), args.GetOffset(), next_substr_case_index); // this may invalidate "str"
				...
				case SCC_SWITCH_CASE: { // {Used to implement case switching}
					...
					str = end;
					str_stack.emplace(&*found->begin(), &*found->end(), ref_param_offset, case_index); // this may invalidate "str"
					break;
				}
