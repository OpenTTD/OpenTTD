...
{
	if (_settings_game.difficulty.terrain_type == MHP_CUSTOM) {
...
static void HeightMapSineTransform(Height h_min, Height h_max)
{
	for (Height &h : _height_map.h) {
		...
		switch (_settings_game.game_creation.average_height) {
			...
			case AVH_LOWLANDS: fheight = SineTransformLowlands(fheight); break;
			case AVH_NORMAL: fheight = SineTransformNormal(fheight); break;
			case AVH_PLATEAUS: fheight = SineTransformPlateaus(fheight); break;
			default: NOT_REACHED();
		}
