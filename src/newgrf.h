...
#define GrfMsg(severity, format_string, ...) do { if ((severity) == Severity::Fatal || IsVisibleSeverity(Facility::Grf, (severity))) GrfMsgI(severity, fmt::format(FMT_STRING(format_string) __VA_OPT__(,) __VA_ARGS__)); } while (false)
