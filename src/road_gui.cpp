	...
	*/
	EventState ChangeRoadTypeOnHotkey(int hotkey)
	{
		...
		switch(SpecialHotkeys(hotkey)) {
			case SpecialHotkeys::FirstRoadtype: break;
			case SpecialHotkeys::LastRoadtype:
				index = _sorted_roadtypes.size() - 1;
				step = _sorted_roadtypes.size() - 1;
				break;
			...
			case SpecialHotkeys::NextRoadtype:
				while (_sorted_roadtypes[index] != this->roadtype) ++index;
		...
		if (this->last_started_action != INVALID_WIDGET) {
			SetCursor(this->GetCursorForWidget(this->last_started_action), PAL_NONE);
