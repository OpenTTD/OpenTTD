...
static CommandCost IsRailStationBridgeAboveOk(TileIndex tile, const StationSpec *statspec, uint8_t layout, int bridge_height, BridgePillarFlags bridge_pillars)
{
	...
	BridgePillarFlags disallowed_pillar_flags;
	if (statspec == nullptr) {
		/* Default stations/waypoints */
		if (layout < 8) {
			disallowed_pillar_flags = default_pillar_flags[layout];
		} else {
			disallowed_pillar_flags = {};
		}
	} else if (layout < std::size(statspec->tilespecs) && statspec->tilespecs[layout].disallowed_pillars != BRIDGEPILLARFLAGS_ALL) {
		/* Pllar flags set by NewGRF */
		disallowed_pillar_flags = statspec->tilespecs[layout].disallowed_pillars;
		...
		disallowed_pillar_flags = default_pillar_flags[layout % 2];
	}

	if (bridge_pillars.Any(disallowed_pillar_flags)) {
...
CommandCost IsRailStationBridgeAboveOk(TileIndex tile, const StationSpec *statspec, uint8_t layout)
{
	...
	BridgePillarFlags bridge_pillars = GetBridgeTilePillarFlags(tile, rampnorth, rampsouth, GetBridgeType(rampsouth), GetTunnelBridgeTransportType(rampsouth));
	return IsRailStationBridgeAboveOk(tile, statspec, layout, GetBridgeHeight(rampsouth), bridge_pillars);
}
...
static CommandCost IsRoadStopBridgeAboveOk(TileIndex tile, const RoadStopSpec *spec, bool drive_through, DiagDirection entrance, int bridge_height, BridgePillarFlags bridge_pillars)
{
	...
	if (spec != nullptr && spec->tilespecs[layout].disallowed_pillars != BRIDGEPILLARFLAGS_ALL) {
		disallowed_pillar_flags = spec->tilespecs[layout].disallowed_pillars;
	} else if (drive_through) {
		disallowed_pillar_flags = DiagDirToAxis(entrance) == AXIS_X ? default_pillar_flags_x : default_pillar_flags_y;
	} else {
		disallowed_pillar_flags.Set(BridgePillarFlag::EdgeNE + entrance);
	}
...
static CommandCost IsRoadStopBridgeAboveOk(TileIndex tile, const RoadStopSpec *spec, bool drive_through, DiagDirection entrance)
{
	...
	TileIndex rampnorth = GetNorthernBridgeEnd(tile);
	BridgePillarFlags bridge_pillars = GetBridgeTilePillarFlags(tile, rampnorth, rampsouth, GetBridgeType(rampsouth), GetTunnelBridgeTransportType(rampsouth));
	return IsRoadStopBridgeAboveOk(tile, spec, drive_through, entrance, GetBridgeHeight(rampsouth), bridge_pillars);
}
...
static CommandCost CheckBuildAbove_Station(TileIndex tile, DoCommandFlags flags, Axis, int height, BridgePillarFlags bridge_pillars)
{
	switch (GetStationType(tile)) {
		case StationType::Rail:
		case StationType::RailWaypoint: {
			CommandCost ret = IsRailStationBridgeAboveOk(tile, GetStationSpec(tile), GetStationGfx(tile), height + 1, bridge_pillars);
			if (ret.Failed()) {
		...
		case StationType::RoadWaypoint: {
			CommandCost ret = IsRoadStopBridgeAboveOk(tile, GetRoadStopSpec(tile), IsDriveThroughStopTile(tile),
					IsDriveThroughStopTile(tile) ? AxisToDiagDir(GetDriveThroughStopAxis(tile)) : GetBayRoadStopDir(tile), height + 1, bridge_pillars);
			if (ret.Failed()) {
				if (ret.GetErrorMessage() != INVALID_STRING_ID) return ret;
				break;
			}
