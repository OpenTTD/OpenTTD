...
/* static */ void Game::ResetConfig()
{
	...
	if (_settings_game.script_config.game != nullptr && _settings_game.script_config.game->HasScript()) {
		if (!_settings_game.script_config.game->ResetInfo(true)) {
			Debug(script, Severity::Fatal, "After a reload, the GameScript by the name '{}' was no longer found, and removed from the list.", _settings_game.script_config.game->GetName());
	...
	if (_settings_newgame.script_config.game != nullptr && _settings_newgame.script_config.game->HasScript()) {
		if (!_settings_newgame.script_config.game->ResetInfo(false)) {
			Debug(script, Severity::Fatal, "After a reload, the GameScript by the name '{}' was no longer found, and removed from the list.", _settings_newgame.script_config.game->GetName());
