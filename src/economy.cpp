...
static void CompaniesGenStatistics()
{
	...
	if (!HasBit(1 << 0 | 1 << 3 | 1 << 6 | 1 << 9, _cur_economy_month)) return;
...
static void CompaniesPayInterest()
{
	...
	for (const Company *c : Company::Iterate()) {
		...
		Money up_to_previous_month = yearly_fee * _cur_economy_month / 12;
		Money up_to_this_month = yearly_fee * (_cur_economy_month + 1) / 12;
...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	for (Vehicle *v = front; v != nullptr; v = v->Next()) {
		...
		ge->last_age = std::min(_cur_economy_year - front->build_year, 255);
...
CommandCost CmdBuyShareInCompany(DoCommandFlag flags, CompanyID target_company)
{
	...
	if (_cur_economy_year - c->inaugurated_economy_year < _settings_game.economy.min_years_for_shares) return_cmd_error(STR_ERROR_PROTECTED);
