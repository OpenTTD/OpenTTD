...
{
	ExternalTownData town = *(ExternalTownData *)user_data;
	return (Command<CMD_FOUND_TOWN>::Post(tile, TSZ_SMALL, town.is_city, _settings_game.economy.town_layout, false, 0, town.name));
}
...
void LoadTownData()
{
	...
	size_t filesize;
	char *text{};
	FILE *handle = FioFOpenFile(_file_to_saveload.name, "rb", TOWN_DATA_DIR, &filesize);
	...
	for (auto &feature : town_data) {
		...
		/* Now that we've created the town, let's grow it to the target population. */
		Town* t = ClosestTownFromTile(search_tile, UINT_MAX);
		uint tries = 500;
