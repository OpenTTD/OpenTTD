...
static std::string convert_tofrom_fs(iconv_t convd, std::string_view name)
{
	...
	if (iconv(convd, &inbuf, &inlen, &outbuf, &outlen) == SIZE_MAX) {
		Debug(Facility::Misc, Severity::Fatal, "[iconv] error converting '{}'. Errno {}", name, errno);
...
static std::optional<iconv_t> OpenIconv(std::string from, std::string to)
{
	...
	if (convd == reinterpret_cast<iconv_t>(-1)) {
		Debug(Facility::Misc, Severity::Fatal, "[iconv] conversion from codeset '{}' to '{}' unsupported", from, to);
		return std::nullopt;
	}
	return convd;
}
...
void OSOpenBrowser(const std::string &url)
{
	...
	Debug(Facility::Misc, Severity::Fatal, "Failed to open url: {}", url);
	exit(0);
}
#endif /* __APPLE__ */
