...
/** Boundary to align strings on section change. */
static constexpr int SECTION_BOUNDARY = 63;
...
/** Deque container to control section name lifetime. */
static std::deque<std::string> _sections = {"", "metadata"};

/**
 * Find an existing section name, or allocate a new one if not previously used.
 * @param section The section name.
 * @return Allocated section name.
 */
static std::string_view AllocateSection(std::string_view section)
{
	/* The section name lifetime exists only in the current filestream buffer.
	 * To avoid storing a copy of the section name for each string, ownership is moved to
	 * a std::deque where it has a controlled lifetime. std::vector cannot be used as
	 * resizing would move strings arounds. */
	auto it = std::ranges::find(_sections, section);
	if (it == std::end(_sections)) it = _sections.emplace(_sections.end(), section);
	return *it;
}
...
void FileStringReader::HandlePragma(std::string_view str, LanguagePackHeader &lang)
{
	...
	if (name == "id") {
		...
		this->data.next_section = AllocateSection(consumer.Read(StringConsumer::npos));
		this->data.next_string_id = (this->data.next_string_id + SECTION_BOUNDARY) & ~SECTION_BOUNDARY;
	} else if (name == "endsection") {
		this->data.next_section = {};
		this->data.next_string_id = (this->data.next_string_id + SECTION_BOUNDARY) & ~SECTION_BOUNDARY;
	} else if (name == "name") {
...
struct HeaderFileWriter : HeaderWriter, FileWriter {
	 ...
	 * Open a file to write to.
	 * @param path The path to the file to open.
	 * @param section The string section to write.
	 * @param total_strings The total number of strings.
	 */
	HeaderFileWriter(const std::filesystem::path &path, std::string_view section, size_t total_strings) : FileWriter("tmp.xxx", std::ofstream::out),
		real_path(path), prev(0), total_strings(total_strings)
	{
		if (section != "") {
			this->section = fmt::format("_{}", section);
			std::ranges::transform(this->section, this->section.begin(), ::toupper);
		}
		this->output_stream << "/**\n";
		...
		this->output_stream << " */\n\n";
		this->output_stream << "#ifndef TABLE_STRINGS" << this->section << "_H\n";
		this->output_stream << "#define TABLE_STRINGS" << this->section << "_H\n";
	}
	...
	void Finalise(const StringData &data, bool stanza) override
	{
		if (stanza) {
		...
		this->output_stream << "#endif /* TABLE_STRINGS" << this->section << "_H */\n";
...
int CDECL main(int argc, char *argv[])
{
	...
	try {
		...
		if (mgo.arguments.empty()) {
			...
			size_t total_strings = std::ranges::count_if(data.strings, [](const auto &ls) { return ls != nullptr; });

			for (std::string_view section : _sections) {
				std::filesystem::path output_path = dest_dir;
				if (section == "") {
					output_path /= "strings.h";
				} else {
					output_path /= fmt::format("strings_{}.h", section);
				}

				HeaderFileWriter writer(output_path, section, total_strings);
				writer.WriteHeader(data, section);
				writer.Finalise(data, section == "metadata");
			}
			if (_strgen.errors != 0) return 1;
		} else {
			std::filesystem::path input_path = std::move(src_dir);
