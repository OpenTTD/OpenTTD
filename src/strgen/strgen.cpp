...
struct FileStringReader : StringReader {
	 ...
	 */
	FileStringReader(StringData &data, const std::filesystem::path &file, bool master, bool translation) : StringReader(data, file.generic_string(), master, translation)
	{
...
static bool CompareFiles(const std::filesystem::path &path1, const std::filesystem::path &path2)
{
	...
	return std::equal(std::istreambuf_iterator<char>(stream1.rdbuf()), std::istreambuf_iterator<char>(), std::istreambuf_iterator<char>(stream2.rdbuf()));
}
...
struct HeaderFileWriter : HeaderWriter, FileWriter {
	 ...
	 * @param path The path to the file to open.
	 */
	HeaderFileWriter(const std::filesystem::path &path) : FileWriter("tmp.xxx", std::ofstream::out), real_path(path), prev(0), total_strings(0)
	{
