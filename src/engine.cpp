...
Engine::Engine(VehicleType type, uint16_t local_id)
{
	...
	if (local_id >= _engine_counts[type]) {
		...
		switch (type) {
			case VEH_TRAIN: this->u.emplace<RailVehicleInfo>(); break;
			case VEH_ROAD: this->u.emplace<RoadVehicleInfo>(); break;
			case VEH_SHIP: this->u.emplace<ShipVehicleInfo>(); break;
			case VEH_AIRCRAFT: this->u.emplace<AircraftVehicleInfo>(); break;
			default: break;
		}
	...
	switch (type) {
		...
		case VEH_TRAIN: {
			RailVehicleInfo &rvi = this->u.emplace<RailVehicleInfo>(_orig_rail_vehicle_info[local_id]);
		...
		case VEH_ROAD: {
			RoadVehicleInfo &rvi = this->u.emplace<RoadVehicleInfo>(_orig_road_vehicle_info[local_id]);
			this->original_image_index = rvi.image_index;
		...
		case VEH_SHIP: {
			ShipVehicleInfo &svi = this->u.emplace<ShipVehicleInfo>(_orig_ship_vehicle_info[local_id]);
		...
		case VEH_AIRCRAFT: {
			AircraftVehicleInfo &avi = this->u.emplace<AircraftVehicleInfo>(_orig_aircraft_vehicle_info[local_id]);
