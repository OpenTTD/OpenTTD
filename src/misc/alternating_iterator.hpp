...
public:
	 ...
	 * @param position Position of the AlternatingIterator.
	 */
	AlternatingIterator(Titer first, Titer last, Titer middle, size_t position) : first(first), last(last), middle(middle), position(position)
	{
		/* Partial iteration is not supported. */
		assert(position == 0 || position == static_cast<size_t>(std::distance(this->first, this->last)));
...
public:
	...
	auto begin() const
	{
		return AlternatingIterator{first, last, middle, 0};
	}

	auto end() const
	{
		return AlternatingIterator{first, last, middle, static_cast<size_t>(std::distance(first, last))};
	}
