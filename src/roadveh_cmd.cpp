...
static const uint16_t _roadveh_images[] = {0xCD4, 0xCDC, 0xCE4, 0xCEC, 0xCF4, 0xCFC, 0xD0C, 0xD14, 0xD24, 0xD1C, 0xD2C, 0xD04, 0xD1C, 0xD24, 0xD6C, 0xD74, 0xD7C, 0xC14, 0xC1C, 0xC24, 0xC2C, 0xC34,
	0xC3C, 0xC4C, 0xC54, 0xC64, 0xC5C, 0xC6C, 0xC44, 0xC5C, 0xC64, 0xCAC, 0xCB4, 0xCBC, 0xD94, 0xD9C, 0xDA4, 0xDAC, 0xDB4, 0xDBC, 0xDCC, 0xDD4, 0xDE4, 0xDDC, 0xDEC, 0xDC4, 0xDDC, 0xDE4, 0xE2C, 0xE34,
	0xE3C, 0xC14, 0xC1C, 0xC2C, 0xC3C, 0xC4C, 0xC5C, 0xC64, 0xC6C, 0xC74, 0xC84, 0xC94, 0xCA4};

static const uint16_t _roadveh_full_adder[] = {0, 88, 0, 0, 0, 0, 48, 48, 48, 48, 0, 0, 64, 64, 0, 16, 16, 0, 88, 0, 0, 0, 0, 48, 48, 48, 48, 0, 0, 64, 64, 0, 16, 16, 0, 88, 0, 0, 0, 0, 48, 48, 48,
	48, 0, 0, 64, 64, 0, 16, 16, 0, 8, 8, 8, 8, 0, 0, 0, 8, 8, 8, 8};
...
static const Trackdir _road_reverse_table[DIAGDIR_END] = {TRACKDIR_RVREV_NE, TRACKDIR_RVREV_SE, TRACKDIR_RVREV_SW, TRACKDIR_RVREV_NW};
...
CommandCost CmdTurnRoadVeh(DoCommandFlags flags, VehicleID veh_id)
{
	...
	if (v->vehstatus.Any({VehState::Stopped, VehState::Crashed}) || v->breakdown_ctr != 0 || v->overtaking != 0 || v->state == RVSB_WORMHOLE || v->IsInDepot() || v->current_order.IsType(OT_LOADING)) {
		return CMD_ERROR;
	}
...
static Vehicle *EnumCheckRoadVehCrashTrain(Vehicle *v, void *data)
{
	...
	return (v->type == VEH_TRAIN && abs(v->z_pos - u->z_pos) <= 6 && abs(v->x_pos - u->x_pos) <= 4 && abs(v->y_pos - u->y_pos) <= 4) ? v : nullptr;
}
...
static void RoadVehCrash(RoadVehicle *v)
{
	...
	EncodedString headline = (victims == 1) ? GetEncodedString(STR_NEWS_ROAD_VEHICLE_CRASH_DRIVER) : GetEncodedString(STR_NEWS_ROAD_VEHICLE_CRASH, victims);
...
static void RoadVehArrivesAt(const RoadVehicle *v, Station *st)
{
	if (v->IsBus()) {
		...
		if (!(st->had_vehicle_of_type & HVOT_BUS)) {
			...
			AddVehicleNewsItem(GetEncodedString(RoadTypeIsRoad(v->roadtype) ? STR_NEWS_FIRST_BUS_ARRIVAL : STR_NEWS_FIRST_PASSENGER_TRAM_ARRIVAL, st->index),
				(v->owner == _local_company) ? NewsType::ArrivalCompany : NewsType::ArrivalOther, v->index, st->index);
		...
		if (!(st->had_vehicle_of_type & HVOT_TRUCK)) {
			...
			AddVehicleNewsItem(GetEncodedString(RoadTypeIsRoad(v->roadtype) ? STR_NEWS_FIRST_TRUCK_ARRIVAL : STR_NEWS_FIRST_CARGO_TRAM_ARRIVAL, st->index),
				(v->owner == _local_company) ? NewsType::ArrivalCompany : NewsType::ArrivalOther, v->index, st->index);
			AI::NewEvent(v->owner, new ScriptEventStationFirstVehicle(st->index, v->index));
			Game::NewEvent(new ScriptEventStationFirstVehicle(st->index, v->index));
		}
...
{
	static const Direction _roadveh_new_dir[] = {DIR_N, DIR_NW, DIR_W, INVALID_DIR, DIR_NE, DIR_N, DIR_SW, INVALID_DIR, DIR_E, DIR_SE, DIR_S};
...
static void RoadVehCheckOvertake(RoadVehicle *v, RoadVehicle *u)
{
	...
	if (u_speed >= v->GetCurrentMaxSpeed() && !u->vehstatus.Test(VehState::Stopped) && u->cur_speed != 0) {
		return;
	}
...
#define return_track(x) { best_track = (Trackdir)x; goto found_best_track; }
	...
	if (IsTileType(tile, MP_ROAD)) {
		...
		if (!IsTileOwner(tile, v->owner) || GetBayRoadStopDir(tile) == enterdir || v->HasArticulatedPart()) {
			...
			if (GetRoadStopType(tile) != rstype) {
				...
				if (!_settings_game.pf.roadveh_queue && IsBayRoadStopTile(tile) && !RoadStop::GetByTile(tile, rstype)->HasFreeBay()) {
	...
	if (v->reverse_ctr != 0) {
		...
		if (RoadTypeIsTram(v->roadtype)) {
			...
			reverse = ((rb & straight) == straight) || (rb == DiagDirToRoadBits(enterdir));
		}
		if (reverse) {
			v->reverse_ctr = 0;
			if (v->tile != tile) {
...
static Trackdir FollowPreviousRoadVehicle(const RoadVehicle *v, const RoadVehicle *prev, TileIndex tile, DiagDirection entry_dir, bool already_reversed)
{
	...
	if (prev_state == RVSB_WORMHOLE || prev_state == RVSB_IN_DEPOT) {
		...
		if (already_reversed && prev->tile != tile) {
			...
			static const Trackdir reversed_turn_lookup[2][DIAGDIR_END] = {
				{TRACKDIR_UPPER_W, TRACKDIR_RIGHT_N, TRACKDIR_LEFT_N, TRACKDIR_UPPER_E}, {TRACKDIR_RIGHT_S, TRACKDIR_LOWER_W, TRACKDIR_LOWER_E, TRACKDIR_LEFT_S}};
	...
	static const RoadBits required_roadbits[] = {ROAD_X, ROAD_Y, ROAD_NW | ROAD_NE, ROAD_SW | ROAD_SE, ROAD_NW | ROAD_SW, ROAD_NE | ROAD_SE, ROAD_X, ROAD_Y};
...
bool IndividualRoadVehicleController(RoadVehicle *v, const RoadVehicle *prev)
{
	...
	RoadDriveEntry rd =
		_road_drive_data[GetRoadTramType(v->roadtype)]
						[((HasBit(v->state, RVS_IN_DT_ROAD_STOP) ? v->state & RVSB_ROAD_STOP_TRACKDIR_MASK : v->state) + (_settings_game.vehicle.road_side << RVS_DRIVE_SIDE)) ^ v->overtaking]
						[v->frame + 1];
	...
	again:
		...
		if (IsReversingRoadTrackdir(dir)) {
			...
			if (RoadTypeIsTram(v->roadtype)) {
				...
				if ((v->Previous() != nullptr && v->Previous()->tile == tile) ||
					(v->IsFrontEngine() && IsNormalRoadTile(tile) && !HasRoadWorks(tile) && HasTileAnyRoadType(tile, v->compatible_roadtypes) && (needed & GetRoadBits(tile, RTT_TRAM)) != ROAD_NONE)) {
					/*
		...
		if (IsInsideMM(v->state, RVSB_IN_ROAD_STOP, RVSB_IN_DT_ROAD_STOP_END) && IsTileType(v->tile, MP_STATION)) {
			...
			if (IsDriveThroughStopTile(v->tile) && RoadStop::IsDriveThroughRoadStopContinuation(v->tile, tile) && v->tile != tile) {
				/* So, keep 'our' state */
	...
	if (v->IsFrontEngine() && !IsInsideMM(v->state, RVSB_IN_ROAD_STOP, RVSB_IN_ROAD_STOP_END)) {
		...
		if (u != nullptr) {
			...
			if (v->cur_speed == 0 && IsInsideMM(v->state, RVSB_IN_DT_ROAD_STOP, RVSB_IN_DT_ROAD_STOP_END) && v->current_order.ShouldStopAtStation(v, GetStationIndex(v->tile)) &&
				v->owner == GetTileOwner(v->tile) && !v->current_order.IsType(OT_LEAVESTATION) && GetRoadStopType(v->tile) == (v->IsBus() ? RoadStopType::Bus : RoadStopType::Truck)) {
	...
	if (v->IsFrontEngine() &&
		((IsInsideMM(v->state, RVSB_IN_ROAD_STOP, RVSB_IN_ROAD_STOP_END) && _road_stop_stop_frame[v->state - RVSB_IN_ROAD_STOP + (_settings_game.vehicle.road_side << RVS_DRIVE_SIDE)] == v->frame) ||
			(IsInsideMM(v->state, RVSB_IN_DT_ROAD_STOP, RVSB_IN_DT_ROAD_STOP_END) && v->current_order.ShouldStopAtStation(v, GetStationIndex(v->tile)) && v->owner == GetTileOwner(v->tile) &&
				GetRoadStopType(v->tile) == (v->IsBus() ? RoadStopType::Bus : RoadStopType::Truck) && v->frame == RVC_DRIVE_THROUGH_STOP_FRAME))) {
...
static void CheckIfRoadVehNeedsService(RoadVehicle *v)
{
	...
	if (v->current_order.IsType(OT_GOTO_DEPOT) && v->current_order.GetNonStopType() & ONSF_NO_STOP_AT_INTERMEDIATE_STATIONS && !Chance16(1, 20)) {
		return;
	}
