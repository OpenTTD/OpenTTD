	...
	bool Load(SoundEntry &sound, bool new_format, std::vector<std::byte> &data) const override
	{
		...
		for (;;) {
			...
			if (tag == WavTag{"fmt "}) {
				...
				if (format != 1) {
					Debug(grf, Severity::Fatal, "SoundLoader_Wav: Unsupported format {}, expected 1 (uncompressed PCM).", format);
					return false;
				}

				sound.channels = file.ReadWord();
				if (sound.channels != 1) {
					Debug(grf, Severity::Fatal, "SoundLoader_Wav: Unsupported channels {}, expected 1.", sound.channels);
					return false;
				}
				...
				if (sound.bits_per_sample != 8 && sound.bits_per_sample != 16) {
					Debug(grf, Severity::Fatal, "SoundLoader_Wav: Unsupported bits_per_sample {}, expected 8 or 16.", sound.bits_per_sample);
					return false;
				}
				...
				if (Align(size, align) != size) {
					...
					Debug(grf, Severity::Fatal, "SoundLoader_Wav: Unexpected end of stream.");
					return false;
				}
