...
static constexpr NWidgetPart _framerate_window_widgets[] = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_CAPTION, COLOUR_GREY, WID_FRW_CAPTION), SetToolTip(STR_TOOLTIP_WINDOW_TITLE_DRAG_THIS),
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			case WID_FRW_CAPTION:
				...
				if (!this->IsShaded()) return GetString(STR_FRAMERATE_CAPTION_SMALL);

				return GetString(STR_FRAMERATE_CAPTION, this->rate_gameloop.strid, this->rate_gameloop.GetValue(), this->rate_gameloop.GetDecimals(), this->speed_gameloop.GetValue(), this->speed_gameloop.GetDecimals());
