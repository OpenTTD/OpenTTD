...
static void PickerLoadConfig(const IniFile &ini, PickerCallbacks &callbacks)
{
	...
	for (auto it = collections.begin(); it != collections.end(); it++) {
		...
		std::string name = GetString(STR_PICKER_DEFAULT_COLLECTION);
		if (collection_pos != std::string_view::npos) name = (*it).substr(collection_pos + 1);

		if (group->items.empty() && collection_pos != std::string_view::npos) {
			callbacks.saved[name];
			continue;
		}
...
static void PickerSaveConfig(IniFile &ini, const PickerCallbacks &callbacks)
{
	...
	for (auto it = callbacks.saved.begin(); it != callbacks.saved.end(); it++) {
		if (it->first == GetString(STR_PICKER_DEFAULT_COLLECTION)) {
			...
			IniGroup &group = ini.GetOrCreateGroup((callbacks.ini_group) + "-" + it->first);
			group.Clear();
			for (const PickerItem &item : it->second) {
				std::string key = fmt::format("{:08X}|{}", std::byteswap(item.grfid), item.local_id);
				group.CreateItem(key);
...
void PickerWindow::ConstructWindow()
{
	...
	if (this->has_collection_picker) {
		if (auto *nwid = this->GetWidget<NWidgetStacked>(WID_PW_TOGGLE_SEL); nwid != nullptr) nwid->SetDisplayedPlane(0);
		this->GetWidget<NWidgetStacked>(WID_PW_COLEC_SEL)->SetDisplayedPlane(this->callbacks.show_collections ? 0 : SZSP_NONE);
		this->GetWidget<NWidgetCore>(WID_PW_COLEC_LIST)->SetToolTip(this->callbacks.GetCollectionTooltip());
	} else {
		if (auto *nwid = this->GetWidget<NWidgetStacked>(WID_PW_TOGGLE_SEL); nwid != nullptr) nwid->SetDisplayedPlane(SZSP_NONE);
	}
...
DropDownList PickerWindow::BuildCollectionDropDownList()
{
	DropDownList list;
	int i = 0;
	/* Sorted collections means Default Collection should always be at collections.begin(), push that first to avoid shading complications.*/
	auto it = collections.begin();
	list.push_back(MakeDropDownListStringItem(GetString(STR_JUST_RAW_STRING, *it), i));

	for (it = it + 1; it != collections.end(); it++) {
		i++;
		list.push_back(MakeDropDownListStringItem(GetString(STR_JUST_RAW_STRING, *it), i, false, this->inactive.contains(*it)));
	}
	return list;
}
...
void PickerWindow::OnClick(Point pt, WidgetID widget, int)
{
	switch (widget) {
		...
		case WID_PW_TOGGLE_SIZE:
			this->callbacks.show_collections = !this->callbacks.show_collections;
			this->GetWidget<NWidgetStacked>(WID_PW_COLEC_SEL)->SetDisplayedPlane(this->callbacks.show_collections ? 0 : SZSP_NONE);
			this->ReInit();
			break;
	...
	static constexpr std::initializer_list<NWidgetPart> picker_class_widgets = {
		NWidget(NWID_SELECTION, INVALID_COLOUR, WID_PW_CLASS_SEL),
			NWidget(NWID_VERTICAL),
				...
				EndContainer(),
				/* Collection view */
				NWidget(NWID_SELECTION, INVALID_COLOUR, WID_PW_COLEC_SEL),
					NWidget(NWID_VERTICAL),
...
/** Create the collection toggle widget. */
std::unique_ptr<NWidgetBase> MakePickerCollectionToggle()
{
	static constexpr std::initializer_list<NWidgetPart> picker_collection_toggle = {
		NWidget(NWID_SELECTION, INVALID_COLOUR, WID_PW_TOGGLE_SEL),
			NWidget(WWT_IMGBTN, COLOUR_DARK_GREEN, WID_PW_TOGGLE_SIZE), SetSpriteTip(SPR_LARGE_SMALL_WINDOW, STR_PICKER_MODE_COLLECTION_TOOLTIP), SetFill(0, 1), SetMinimalSize(14, 14), SetAspect(WidgetDimensions::ASPECT_TOGGLE_SIZE),
		EndContainer(),
	};

	return MakeNWidgets(picker_collection_toggle, nullptr);
}
