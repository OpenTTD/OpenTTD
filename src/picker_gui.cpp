...
void PickerWindow::UpdateWidgetSize(WidgetID widget, Dimension &size, const Dimension &padding, Dimension &fill, Dimension &resize)
{
	switch (widget) {
		...
		case WID_PW_CONFIGURE_BADGES:
			if (this->badge_classes.GetClasses().empty()) size.width = 0;
			break;
	}
}
...
std::string PickerWindow::GetWidgetString(WidgetID widget, StringID stringid) const
{
	if (IsInsideMM(widget, this->badge_filters.first, this->badge_filters.second)) {
		return GetBadgeDropdownFilterString(*this->GetWidget<NWidgetBase>(widget), this->badge_filter_conf);
	}
...
void PickerWindow::OnClick(Point pt, WidgetID widget, int)
{
	switch (widget) {
		...
		default:
			if (IsInsideMM(widget, this->badge_filters.first, this->badge_filters.second)) {
				ShowDropDownList(this, GetBadgeDropdownFilterList(*this->GetWidget<NWidgetBase>(widget)), -1, widget, 0, false);
			}
			break;
	}
}

void PickerWindow::OnDropdownSelect(WidgetID widget, int index)
{
	switch (widget) {
		case WID_PW_CONFIGURE_BADGES: {
			if (index < 0) return;
			if (this->badge_classes.GetClasses().empty()) return;
			...
			HandleBadgeConfigurationDropDownClick(feature, BadgeClassID(index), 1);
		...
		default:
			if (IsInsideMM(widget, this->badge_filters.first, this->badge_filters.second)) {
				if (index < 0) {
					ResetBadgeFilter(this->badge_filter_conf, GetBadgeDropdownFilterClass(*this->GetWidget<NWidgetBase>(widget)));
				} else {
					SetBadgeFilter(this->badge_filter_conf, BadgeID(index));
				}
				this->type_string_filter.bdf.emplace(this->badge_filter_conf);
