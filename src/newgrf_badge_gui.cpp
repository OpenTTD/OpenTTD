...
public:
	...
	void Draw(const Rect &full, const Rect &r, bool sel, int click_result, Colours bg_colour) const override
	{
		...
		Rect br = r.WithWidth(w, TEnd ^ rtl).CentreTo(w, SETTING_BUTTON_HEIGHT);
		DrawArrowButtons(br.left, br.top, this->button_colour, state, this->click_up, this->click_down);
...
static constexpr int CLICK_MOVE_UP = 1;
static constexpr int CLICK_MOVE_DOWN = 2;
static constexpr int CLICK_TOGGLE_ICON = 3;
...
DropDownList BuildBadgeClassConfigurationList(const GUIBadgeClasses &gui_classes, uint columns, std::span<const StringID> column_separators)
{
	...
	for (uint i = 0; i < columns; ++i) {
		for (const auto &gc : gui_classes.GetClasses()) {
			...
			list.push_back(std::make_unique<DropDownListToggleMoverItem>(first ? 0 : CLICK_MOVE_UP, last ? 0 : CLICK_MOVE_DOWN, COLOUR_YELLOW, gc.visible, CLICK_TOGGLE_ICON, COLOUR_YELLOW, COLOUR_GREY, GetString(GetClassBadge(gc.class_index)->name), gc.class_index.base()));
		}
...
static void BadgeClassToggleVisibility(GrfSpecFeature feature, Badge &class_badge, int click_result)
{
	...
	if (click_result == CLICK_TOGGLE_ICON) it->show_icon = !it->show_icon;
}
...
bool HandleBadgeConfigurationDropDownClick(GrfSpecFeature feature, uint columns, int result, int click_result)
{
	...
	switch (click_result) {
		case CLICK_MOVE_DOWN: // Move down button.
			BadgeClassMoveNext(feature, *class_badge, columns);
			break;
		case CLICK_MOVE_UP: // Move up button.
