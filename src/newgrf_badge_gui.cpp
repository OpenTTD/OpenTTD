...
{
	return MakeDropDownListBadgeItem(gui_classes, badges, feature, introduction_date, "", std::move(str), value, masked, shaded, ctrl_bg_colour, shift_bg_colour);
}

std::unique_ptr<DropDownListItem> MakeDropDownListBadgeItem(const std::shared_ptr<GUIBadgeClasses> &gui_classes, std::span<const BadgeID> badges, GrfSpecFeature feature, std::optional<TimerGameCalendar::Date> introduction_date, Money cost, std::string &&str, int value, bool masked, bool shaded, std::optional<Colours> ctrl_bg_colour, std::optional<Colours> shift_bg_colour)
{
	return MakeDropDownListBadgeItem(gui_classes, badges, feature, introduction_date, GetString(STR_JUST_CURRENCY_SHORT, cost), std::move(str), value, masked, shaded, ctrl_bg_colour, shift_bg_colour);
}
