...
/**
	...
	*/
/* static */ void FontCache::AddFallback(FontSizes fontsizes, const std::string &name, std::span<const std::byte> os_data)
{
	for (FontSize fs : fontsizes) {
		GetFontCacheSubSetting(fs)->fallback_fonts.emplace_back(name, std::vector<std::byte>{os_data.begin(), os_data.end()});
	}
}
