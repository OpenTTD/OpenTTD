 ...
 */
TrueTypeFontCache::TrueTypeFontCache(FontSize fs, int pixels) : FontCache(fs), req_size(pixels), glyph_to_sprite(NULL)
{
}
...
private:
	...
	HDC dc = NULL;      ///< Cached GDI device context.
...
void Win32FontCache::SetFontSize(FontSize fs, int pixels)
{
	if (pixels == 0) {
		...
		if (temp != NULL) {
			...
			UINT size = GetOutlineTextMetrics(this->dc, 0, NULL);
	...
	UINT otmSize = GetOutlineTextMetrics(this->dc, 0, NULL);
...
void Win32FontCache::ClearFontCache()
{
	...
	/* GUI scaling might have changed, determine font size anew if it was automatically selected. */
	if (this->font != NULL && this->req_size == 0) this->SetFontSize(this->fs, this->req_size);
}
...
/* virtual */ const Sprite *Win32FontCache::InternalGetGlyph(GlyphID key, bool aa)
{
	...
	if (size == GDI_ERROR) {
		/* No dice with the guess. First query size of needed glyph memory, then allocate
		 * the memory and query again. This dance is necessary as some glyphs will only with
		 * the exact matching size; e.g. the space glyph has no pixels and must be requested
		 * with size == 0, anything else fails. Unfortunately, a failed call doesn't return
		 * any info about the size and thus the triple GetGlyphOutline()-call. */
		size = GetGlyphOutline(this->dc, key, GGO_GLYPH_INDEX | (aa ? GGO_GRAY8_BITMAP : GGO_BITMAP), &gm, 0, NULL, &mat);
...
{
	DWORD len = GetFontData(this->dc, tag, 0, NULL, 0);
...
static void LoadWin32Font(FontSize fs)
{
	...
	LOGFONT logfont = { 0 };
	if (settings->os_handle != NULL) logfont = *(const LPLOGFONT)settings->os_handle;

	if (logfont.lfFaceName[0] == 0) {
		logfont.lfWidth = 0;
		logfont.lfEscapement = 0;
		logfont.lfOrientation = 0;
		logfont.lfWeight = strcasestr(settings->font, " bold") != NULL ? FW_BOLD : FW_NORMAL; // Poor man's way to allow selecting bold fonts.
