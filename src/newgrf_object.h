...
struct ObjectSpec : NewGRFSpecBase<ObjectClassID> {
	...
	{
		return GetPrice(PR_BUILD_OBJECT, this->build_cost_multiplier, this->grf_prop.grffile, 0);
	}
	...
	{
		return GetPrice(PR_CLEAR_OBJECT, this->clear_cost_multiplier, this->grf_prop.grffile, 0);
	}
...
struct ObjectScopeResolver : public ScopeResolver {
	 ...
	 */
	ObjectScopeResolver(ResolverObject &ro, Object *obj, const ObjectSpec *spec, TileIndex tile, uint8_t view = 0) : ScopeResolver(ro), obj(obj), spec(spec), tile(tile), view(view) {}
