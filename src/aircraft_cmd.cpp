...
static StationID FindNearestHangar(const Aircraft *v)
{
	...
	FOR_ALL_STATIONS(st) {
		...
		if (max_range != 0) {
			...
			uint last_dist = last_dest != NULL && last_dest->airport.tile != INVALID_TILE ? DistanceSquare(st->airport.tile, last_dest->airport.tile) : 0;
			uint next_dist = next_dest != NULL && next_dest->airport.tile != INVALID_TILE ? DistanceSquare(st->airport.tile, next_dest->airport.tile) : 0;
