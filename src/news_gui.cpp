...
static const uint MAX_NEWS_AMOUNT = 1 << 10;   ///< Do not exceed this number of news messages.
...
{
	auto first = std::begin(_news);
	auto last = std::next(first, std::min(count, std::size(_news)));
...
{
	for (auto ni = std::begin(_news); ni != std::end(_news); ++ni) {
...
void ShowLastNewsMessage()
{
	...
	if (_forced_news == std::end(_news)) {
		...
		if (_current_news == std::end(_news)) {
			 ...
			 * Treat this as if _forced_news reached _oldest_news; so, wrap around and start anew with the latest. */
			ni = std::begin(_news);
		} else {
