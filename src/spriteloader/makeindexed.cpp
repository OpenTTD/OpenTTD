	...
	for (auto *pixel = sprite.data; pixel != pixel_end; ++pixel) {
		if (pixel->m != 0) {
			...
			if (IsInsideMM(pixel->m, 0xC6, 0xCE)) {
				// uint sq1000_brightness = IntSqrt((c.r * c.r * 299 + c.g * c.g * 587 + c.b * c.b * 114) / 1000);
			...
			/* Find nearest match from palette o_O */
			pixel->m = GetNearestColourIndex(pixel->r, pixel->g, pixel->b);
		}
	}
}

uint8_t SpriteLoaderMakeIndexed::LoadSprite(SpriteLoader::SpriteCollection &sprite, SpriteFile &file, size_t file_pos, SpriteType sprite_type, bool, byte control_flags, uint8_t &avail_8bpp, uint8_t &avail_32bpp)
{
