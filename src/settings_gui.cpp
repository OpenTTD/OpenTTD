...
static const int FONT_HALF_GAP = DEFAULT_FONT_MIN_MAX_GAP / 2;
static const int FONT_SLIDER_MARK_COUNT = DEFAULT_FONT_MIN_MAX_GAP + 1; ///< Labeled at every other number. 8 Labeled marks and 7 unlabeled ones.
...
static int CalculateFontSliderMin(int current_size, int minimum_size)
{
	...
	int first_pass_minimum = std::max(current_size - FONT_HALF_GAP, minimum_size);

	/* Adjust for the cases where marks are missing because the top end of the slider has hit the max value: */
	return std::min(first_pass_minimum, DEFAULT_FONT_MAX_HEIGHT - DEFAULT_FONT_MIN_MAX_GAP);
}
...
static int CalculateFontSliderMax(int current_size, int minimum_size)
{
	...
	int first_pass_maximum = std::min(current_size + FONT_HALF_GAP, DEFAULT_FONT_MAX_HEIGHT);

	/* Adjust for the cases where marks are missing because the bottom end of the slider has hit the minimum value: */
	return std::max(first_pass_maximum, minimum_size + DEFAULT_FONT_MIN_MAX_GAP);
}
