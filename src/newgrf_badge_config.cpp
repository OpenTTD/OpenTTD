...
public:
	...
	static constexpr GrfSpecFeatures CONFIGURABLE_FEATURES = {
		GSF_TRAINS,     GSF_ROADVEHICLES, GSF_SHIPS,   GSF_AIRCRAFT,  GSF_STATIONS,     GSF_BRIDGES,   GSF_HOUSES,    GSF_INDUSTRYTILES,
		GSF_INDUSTRIES, GSF_AIRPORTS,     GSF_OBJECTS, GSF_RAILTYPES, GSF_AIRPORTTILES, GSF_ROADTYPES, GSF_TRAMTYPES, GSF_ROADSTOPS,
	};
	...
	static inline const std::array<std::string_view, GrfSpecFeature::GSF_END> sections = {
		...
		{}, // GSF_CANALS
		"badges_bridges", // GSF_BRIDGES
		"badges_houses", // GSF_HOUSES
		{}, // GSF_GLOBALVAR
		"badges_industrytiles", // GSF_INDUSTRYTILES
		"badges_industries", // GSF_INDUSTRIES
		{}, // GSF_CARGOES
		{}, // GSF_SOUNDFX
		...
		"badges_airporttiles", // GSF_AIRPORTTILES
...
void AddBadgeClassesToConfiguration()
{
	for (const GrfSpecFeature &feature : BadgeClassConfig::CONFIGURABLE_FEATURES) {
		...
		for (const BadgeID &index : GetClassBadges()) {
			...
			/* Not found, insert it. */
			config.emplace_back(badge.label, 0, 0, true, false);
		}
	}
}
...
{
	static const BadgeClassConfigItem empty{}; ///< empty item if the label is not present or configurable.

	if (!BadgeClassConfig::CONFIGURABLE_FEATURES.Test(feature)) return empty;

	auto config = GetBadgeClassConfiguration(feature);
	auto found = std::ranges::find(config, label, &BadgeClassConfigItem::label);
	if (found == std::end(config)) return empty;

	/* Sort order is simply the position in the configuration list. */
	found->sort_order = static_cast<int>(std::distance(std::begin(config), found));
	return *found;
}
...
static void BadgeClassLoadConfigFeature(const IniFile &ini, GrfSpecFeature feature)
{
	...
	for (const IniItem &item : group->items) {
		...
		config.emplace_back(item.name, column, -1, show_icon, show_filter);
	}
}
