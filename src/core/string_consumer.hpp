 ...
 *
 * There are generally three operations for each data type:
...
public:
	...
	[[nodiscard]] std::string_view Read(size_type len)
	{
		...
		if (len != npos && len != result.size()) {
			this->LogError(fmt::format("Source buffer too short: {} > {}", len, result.size()));
		}
	...
	[[nodiscard]] static std::pair<size_type, T> ParseIntegerBase(std::string_view src, int base, bool log_errors)
	{
		if (base == 0) {
			if (src.starts_with("0x") || src.starts_with("0X")) {
				...
				return {len + 2, value};
			} else if (std::is_signed_v<T> && (src.starts_with("-0x") || src.starts_with("-0X"))) {
