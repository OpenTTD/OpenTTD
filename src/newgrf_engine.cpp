...
static uint32 VehicleGetVariable(Vehicle *v, const VehicleScopeResolver *object, byte variable, uint32 parameter, bool *available)
{
	...
	switch (variable - 0x80) {
		...
		case 0x52: return ClampToI32(v->GetDisplayProfitThisYear().RawValue());
		case 0x53: return GB(ClampToI32(v->GetDisplayProfitThisYear().RawValue()),  8, 24);
		case 0x54: return GB(ClampToI32(v->GetDisplayProfitThisYear().RawValue()), 16, 16);
		case 0x55: return GB(ClampToI32(v->GetDisplayProfitThisYear().RawValue()), 24,  8);
		case 0x56: return ClampToI32(v->GetDisplayProfitLastYear().RawValue());
		case 0x57: return GB(ClampToI32(v->GetDisplayProfitLastYear().RawValue()),  8, 24);
		case 0x58: return GB(ClampToI32(v->GetDisplayProfitLastYear().RawValue()), 16, 16);
		case 0x59: return GB(ClampToI32(v->GetDisplayProfitLastYear().RawValue()), 24,  8);
		case 0x5A: return v->Next() == nullptr ? INVALID_VEHICLE : v->Next()->index;
		case 0x5C: return ClampToI32(v->value.RawValue());
		case 0x5D: return GB(ClampToI32(v->value.RawValue()),  8, 24);
		case 0x5E: return GB(ClampToI32(v->value.RawValue()), 16, 16);
		case 0x5F: return GB(ClampToI32(v->value.RawValue()), 24,  8);
