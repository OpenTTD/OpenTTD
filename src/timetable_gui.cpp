...
StringID GetTimetableStringFormat()
{
	switch (_settings_client.gui.timetable_mode) {
		case TimetableMode::Days: return STR_UNITS_DAYS;
		case TimetableMode::Seconds: return STR_UNITS_SECONDS;
		case TimetableMode::Ticks: return STR_UNITS_TICKS;
		default:
			NOT_REACHED();
	}
}

uint64_t GetTimetableStringValue(TimerGameTick::Ticks ticks)
{
	switch (_settings_client.gui.timetable_mode) {
		case TimetableMode::Days: return ticks / Ticks::DAY_TICKS;
		case TimetableMode::Seconds: return ticks / Ticks::TICKS_PER_SECOND;
		case TimetableMode::Ticks: return ticks;
		default:
			NOT_REACHED();
	}
}
...
struct TimetableWindow : Window {
		...
		while (order != nullptr) {
			...
			if (i % 2 == 0) {
				...
				if (order->IsType(OT_CONDITIONAL)) {
					text = GetString(STR_TIMETABLE_NO_TRAVEL);
				} else if (order->IsType(OT_IMPLICIT)) {
					text = GetString(STR_TIMETABLE_NOT_TIMETABLEABLE);
					colour = ((i == selected) ? TC_SILVER : TC_GREY) | TC_NO_SHADE;
				} else if (!order->IsTravelTimetabled()) {
					if (order->GetTravelTime() > 0) {
						text = order->GetMaxSpeed() != UINT16_MAX
							? GetString(STR_TIMETABLE_TRAVEL_FOR_SPEED_ESTIMATED,
								GetTimetableStringFormat(), GetTimetableStringValue(order->GetTravelTime()), PackVelocity(order->GetMaxSpeed(), v->type))
							: GetString(STR_TIMETABLE_TRAVEL_FOR_ESTIMATED,
								GetTimetableStringFormat(), GetTimetableStringValue(order->GetTravelTime()));
					} else {
						text = order->GetMaxSpeed() != UINT16_MAX
							? GetString(STR_TIMETABLE_TRAVEL_NOT_TIMETABLED_SPEED,
								PackVelocity(order->GetMaxSpeed(), v->type))
							: GetString(STR_TIMETABLE_TRAVEL_NOT_TIMETABLED);
					}
				} else {
					text = order->GetMaxSpeed() != UINT16_MAX
						? GetString(STR_TIMETABLE_TRAVEL_FOR_SPEED,
							GetTimetableStringFormat(), GetTimetableStringValue(order->GetTimetabledTravel()), PackVelocity(order->GetMaxSpeed(), v->type))
						: GetString(STR_TIMETABLE_TRAVEL_FOR,
							GetTimetableStringFormat(), GetTimetableStringValue(order->GetTimetabledTravel()), PackVelocity(order->GetMaxSpeed(), v->type));
				}

				DrawString(rtl ? tr.left : middle, rtl ? middle : tr.right, tr.top, text, colour);
	...
	void DrawSummaryPanel(const Rect &r) const
	{
		...
		if (total_time != 0) {
			...
			DrawString(tr, GetString(str, GetTimetableStringFormat(), GetTimetableStringValue(total_time)));
		}
		tr.top += GetCharacterHeight(FS_NORMAL);
		...
		if (v->timetable_start != 0) {
			...
			/* We are late. */
			DrawString(tr, GetString(v->lateness_counter < 0 ? STR_TIMETABLE_STATUS_EARLY : STR_TIMETABLE_STATUS_LATE,
				GetTimetableStringFormat(), GetTimetableStringValue(v->lateness_counter)));
		}
	}
