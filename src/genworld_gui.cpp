...
struct GenerateLandscapeWindow : public Window {
	...
	GenerateLandscapeWindow(WindowDesc &desc, WindowNumber number = 0) : Window(desc)
	{
		...
		if (_settings_newgame.game_creation.land_generator == LG_ORIGINAL) {
			_settings_newgame.difficulty.terrain_type = Clamp(_settings_newgame.difficulty.terrain_type, MHP_VERY_FLAT, MHP_MOUNTAINOUS);
		}
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			...
			case WID_GL_MAX_HEIGHT_PULLDOWN:
				if (_settings_newgame.difficulty.terrain_type == MHP_CUSTOM) {
					return GetString(STR_TERRAIN_TYPE_CUSTOM_VALUE, _settings_newgame.game_creation.custom_terrain_type);
				}
				return GetString(_max_height[_settings_newgame.difficulty.terrain_type]);
			...
			case WID_GL_AVERAGE_HEIGHT_PULLDOWN: return GetString(_average_height[_settings_newgame.game_creation.average_height]);
	...
	void OnInvalidateData([[maybe_unused]] int data = 0, [[maybe_unused]] bool gui_scope = true) override
	{
		...
		if (_settings_newgame.game_creation.land_generator == LG_ORIGINAL) {
			...
			if (_settings_newgame.difficulty.terrain_type == MHP_CUSTOM) {
				_settings_newgame.difficulty.terrain_type = MHP_FLAT;
			}
		}
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_GL_MAX_HEIGHT_PULLDOWN: // Max height
				...
				ShowDropDownMenu(this, _max_height, _settings_newgame.difficulty.terrain_type, WID_GL_MAX_HEIGHT_PULLDOWN, 0, _settings_newgame.game_creation.land_generator == LG_ORIGINAL ? ~0xF : 0);
				break;
			...
			case WID_GL_AVERAGE_HEIGHT_PULLDOWN: // Average height
				ShowDropDownMenu(this, _average_height, _settings_newgame.game_creation.average_height, WID_GL_AVERAGE_HEIGHT_PULLDOWN, 0, 0);
				break;
	...
	void OnDropdownSelect(WidgetID widget, int index, int) override
	{
		switch (widget) {
			...
			case WID_GL_AVERAGE_HEIGHT_PULLDOWN: _settings_newgame.game_creation.average_height = index; break;
			...
			case WID_GL_MAX_HEIGHT_PULLDOWN: {
				if (static_cast<MaxHeightPreset>(index) == MHP_CUSTOM) {
					this->widget_id = widget;
