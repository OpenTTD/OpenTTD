 ...
 */
template <typename T>
static ChangeInfoResult LoadTranslationTable(uint gvid, int numinfo, ByteReader &buf, std::function<std::vector<T> &(GRFFile &)> gettable, std::string_view name)
{
	if (gvid != 0) {
	...
	GRFFile *override = GetCurrentGRFOverride();
	if (override != nullptr) {
		/* GRF override is present, copy the translation table to the overridden GRF as well. */
		GrfMsg(1, "LoadTranslationTable: Copying {} translation table to override GRFID '{}'", name, BSWAP32(override->grfid));
		std::vector<T> &override_table = gettable(*override);
