...
std::optional<std::string_view> SoundDriver_Allegro::Start(const StringList &parm)
{
	if (_allegro_instance_count == 0 && install_allegro(SYSTEM_AUTODETECT, &errno, nullptr)) {
		Debug(Facility::Driver, Severity::Fatal, "allegro: install_allegro failed '{}'", allegro_error);
	...
	if (install_sound(DIGI_AUTODETECT, MIDI_AUTODETECT, nullptr) != 0) {
		Debug(Facility::Driver, Severity::Fatal, "allegro: install_sound failed '{}'", allegro_error);
		return "Failed to set up Allegro sound";
	}

	/* Okay, there's no soundcard */
	if (digi_card == DIGI_NONE) {
		Debug(Facility::Driver, Severity::Fatal, "allegro: no sound card found");
