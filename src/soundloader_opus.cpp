...
public:
	...
	bool Load(SoundEntry &sound, bool new_format, std::vector<std::byte> &data) const override
	{
		...
		if (error != 0) {
			Debug(grf, Severity::Fatal, "SoundLoader_Opus: Unable to open stream.");
			return false;
		}
		...
		for (;;) {
			...
			if (read < 0) {
				Debug(grf, Severity::Fatal, "SoundLoader_Opus: Unexpected end of stream.");
				data.clear();
				return false;
			}
			...
			if (channels != 1) {
				Debug(grf, Severity::Fatal, "SoundLoader_Opus: Unsupported channels {}, expected 1.", channels);
				data.clear();
				return false;
			}
