...
{
	typename T::CharType *buff_begin;
	line.buffer = Layouter::LineCacheItem::Buffer(buff_begin = new typename T::CharType[str.size() + 1],
		[](void *p) { delete[] reinterpret_cast<T::CharType *>(p); });
